...
static volatile uint8_t	MagicCodeBuf[24]  __attribute__((at(0x20000000)));

static uint8_t		updatFwSwTimerRunCount10ms = 0;
static	uint8_t		WaitFwCheckTime = 0;

 
static void updatFwSwTimerHandler( void *dest, uint16_t evt, void *vDataPtr)
{
	uint8_t		FwStatus;
	...
	else if(evt == LIB_SW_TIMER_EVT_SW_10MS_8)
	{
		...
		{
			LibSwTimerClose(updatFwSwTimerHandler, 0);
			appSerialCanDavinciSendTextMessage("Fw Check Fail SwTimer Close");
		}
	}
	else if(evt == LIB_SW_TIMER_EVT_SW_1S)
	{
		if(WaitFwCheckTime)
		{
			...
			{
				LibSwTimerClose(updatFwSwTimerHandler, 0);
				appSerialCanDavinciSendTextMessage("updatFwSwTimerHandler Close");
			}
		}
	}
}

static void resetAndUpdateSwTimerHandler( void *dest, uint16_t evt, void *vDataPtr)
{
...
static void resetAppSwTimerHandler( void *dest, uint16_t evt, void *vDataPtr)
{
	static uint8_t	count =0;
	if(evt == LIB_SW_TIMER_EVT_SW_1MS)
		;
...
{
#line 204 "..\\..\\..\\API\\ApiFu.c"
}


void apiFuJumpToBootloader(void)
{
#line 230 "..\\..\\..\\API\\ApiFu.c"
}
...
void apiFuUpdateFw(void)
{
	...
	if(status == ((int8_t)(0x00) - 0))
	{
		updatFwSwTimerRunCount10ms = 100;
		WaitFwCheckTime = 20;
		appSerialCanDavinciSendTextMessage("Sw Timer Open Success");
	}
	else
		appSerialCanDavinciSendTextMessage("Sw Timer Open fail");
}
