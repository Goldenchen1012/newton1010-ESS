...
#line 18 "..\\..\\..\\API\\ApiModbusTCPIP.c"
uint8_t ModbusTCPIP_buffer[2000];
uint8_t ResponseLength;
#line 30 "..\\..\\..\\API\\ApiModbusTCPIP.c"
W5500_Socket_parm Socket_ModbusTCPIP = { . Num = 0, . Protocol = 0x01, . PortNum = 502, . DeviceID = 1, . Memory . rx_buf_Ptr = ModbusTCPIP_buffer, . Memory . rx_buf_size = 2000, . Memory . tx_buf_Ptr = ModbusTCPIP_buffer, . Memory . tx_buf_size = 2000 };	
uint8_t ModbusTest1_buffer[2000];
#line 42 "..\\..\\..\\API\\ApiModbusTCPIP.c"
W5500_Socket_parm Socket_Test1 = { . Num = 0, . Protocol = 0x01, . PortNum = 5000, . DeviceID = 1, . Memory . rx_buf_Ptr = ModbusTest1_buffer, . Memory . rx_buf_size = 2000, . Memory . tx_buf_Ptr = ModbusTest1_buffer, . Memory . tx_buf_size = 2000 };	
uint8_t ModbusTest2_buffer[2000];
#line 54 "..\\..\\..\\API\\ApiModbusTCPIP.c"
W5500_Socket_parm Socket_Test2 = { . Num = 0, . Protocol = 0x01, . PortNum = 5200, . DeviceID = 1, . Memory . rx_buf_Ptr = ModbusTest2_buffer, . Memory . rx_buf_size = 2000, . Memory . tx_buf_Ptr = ModbusTest2_buffer, . Memory . tx_buf_size = 2000 };	
...
uint16_t Modbus_TCPIP_Parser(W5500_Socket_parm Socket, uint16_t DataLen){
	...
	TempDataPtr = Socket.Memory.rx_buf_Ptr + 2;
	 
	if(ByteToWord(*TempDataPtr,*(TempDataPtr+1))!= 0x0000){
		
		lu8ExceptionCode = Gateway_Target_ResponseFail;
	}else{
		 
		TempDataPtr = Socket.Memory.rx_buf_Ptr + 4;
		if(ByteToWord(*TempDataPtr,*(TempDataPtr+1))!= (DataLen-6)){
			lu8ExceptionCode = Illegal_DataValue;
		}
		 
		TempDataPtr = Socket.Memory.rx_buf_Ptr + 6;
		if(*TempDataPtr != Socket.DeviceID ){
			lu8ExceptionCode = Gateway_Target_ResponseFail;
		}	
		if(lu8ExceptionCode == Normal){
			 
			TempDataPtr = Socket.Memory.rx_buf_Ptr + 7;
		...
		if(lu8ExceptionCode != Normal){
			...
			TempDataPtr = Socket.Memory.tx_buf_Ptr + 4;
			*TempDataPtr = WordToByte(3, HIGH_Byte);
			*(TempDataPtr+1) = WordToByte(3, LOW_Byte);
			 
			TempDataPtr = Socket.Memory.tx_buf_Ptr + 7;
			*TempDataPtr |= 0x80;
...
uint8_t ModbusTCPIP_Func_ReadHoldReg( uint16_t xu16StartRegAddr, uint16_t xu16ReadRegCnt, W5500_Socket_parm Socket){
	...
	if(xu16ReadRegCnt > 127){
		...
		*TempDataPtr = (uint8_t)lu16ByteCnt;
		...
		TempDataPtr = Socket.Memory.tx_buf_Ptr + 9;   
		for(i = 0; i<lu16ByteCnt; i++){
			*(TempDataPtr+i) = 0;  
		}	
	}
	ResponseLength = lu16ByteCnt+3;
	return Normal;
}
...
uint8_t ModbusTCPIP_Func_WriteMultiReg(uint16_t xu16StartRegAddr, uint16_t xu16WriteRegCnt, W5500_Socket_parm Socket){
	...
	TempDataPtr = Socket.Memory.rx_buf_Ptr + 12;
	lu8ByteCnt = *TempDataPtr;
	
	
	if((xu16WriteRegCnt > 127)||(xu16WriteRegCnt == 0)||((xu16WriteRegCnt*2) != (uint16_t)lu8ByteCnt)){
		return Illegal_DataValue;
	}else{
