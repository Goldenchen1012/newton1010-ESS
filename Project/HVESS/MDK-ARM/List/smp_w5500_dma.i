...
#line 18 "..\\..\\..\\Drivers\\SMP\\smp_w5500_DMA.c"
...
#line 19 "..\\..\\..\\Drivers\\SMP\\smp_w5500_DMA.c"
#line 20 "..\\..\\..\\Drivers\\SMP\\smp_w5500_DMA.c"
#line 21 "..\\..\\..\\Drivers\\SMP\\smp_w5500_DMA.c"
...
volatile uint16_t SocketHandleByteLen[8] = {0,0,0,0,0,0,0,0};
volatile uint16_t gu16Temp;
static uint8_t socketStatus[8];
static uint8_t SocketEnableCount = 0;
uint8_t Socket_Event_Handling_Status[8];

static uint8_t gu8W5500_SPI_Status = W5500_SPI_Idle;
static uint8_t gu8_W5500_Init_Steps = Init_End;
static uint8_t gu8Socket_Sever_Step[8] = {0};

static uint8_t DefMac[6] = {0x00, 0xf1, 0xbe, 0xc4, 0xa1, 0x05};
static uint8_t DefIp[4] = {192, 168, 1, 16};
static uint8_t DefSn[4] = {255, 255, 255, 0};
static uint8_t DefGw[4] = {192, 168, 1, 1};
static uint8_t DefDns[4] = {8, 8, 8, 8};

uint8_t W5500_tx_Buf[256];
...
W5500_RegisterList smpW5500Socket[8];
...
void EXTI9_5_IRQHandler(void)
{
    ...
    HAL_GPIO_EXTI_IRQHandler(((uint16_t)0x0080));
...
static void W5500SwTimerHandler( void *dest, uint16_t evt, void *vDataPtr){
	static uint8_t SocketNum = 0;
	static uint8_t EventDoneCheck = 0;
	if(evt == LIB_SW_TIMER_EVT_SW_1MS){
		if(gu8_W5500_Init_Steps != Init_End){
			HAL_GPIO_WritePin(((GPIO_TypeDef *) (((0x40000000UL) + 0x08000000UL) + 0x0C00UL)), ((uint16_t)0x0010), GPIO_PIN_SET);
			W5500_Init_Step(gu8_W5500_Init_Steps);
			HAL_GPIO_WritePin(((GPIO_TypeDef *) (((0x40000000UL) + 0x08000000UL) + 0x0C00UL)), ((uint16_t)0x0010), GPIO_PIN_RESET);
		}else{
			if(W5500INT_Low){
				HAL_GPIO_WritePin(((GPIO_TypeDef *) (((0x40000000UL) + 0x08000000UL) + 0x0C00UL)), ((uint16_t)0x0010), GPIO_PIN_SET);
...
void smp_w5500_getMAC(void){
	if(gu8W5500_SPI_Status == W5500_SPI_Idle){
		smp_w5500_spiDMA_ReadMulti((0x00000000 + (0x0009 << 8) + (0x00 << 3)), 6, TmpNetInfo.mac);	
	}
}

void smp_w5500_getGW(void){
	if(gu8W5500_SPI_Status == W5500_SPI_Idle){
		smp_w5500_spiDMA_ReadMulti((0x00000000 + (0x0001 << 8) + (0x00 << 3)), 4,  TmpNetInfo.gw);	
	}
}
void smp_w5500_getSUBMASK(void){
	if(gu8W5500_SPI_Status == W5500_SPI_Idle){
		smp_w5500_spiDMA_ReadMulti((0x00000000 + (0x0005 << 8) + (0x00 << 3)), 4,  TmpNetInfo.sn);	
	}
}
void smp_w5500_getLocalIP(void){
	if(gu8W5500_SPI_Status == W5500_SPI_Idle){
		smp_w5500_spiDMA_ReadMulti((0x00000000 + (0x000F << 8) + (0x00 << 3)), 4,  TmpNetInfo.ip);	
	}
}
...
#line 288 "..\\..\\..\\Drivers\\SMP\\smp_w5500_DMA.c"
	...
	smp_spi_master_init(&SPI_W5500, spi_master_w5500_event_handler, 0);
...
int8_t W5500_Init_Step(uint8_t Step){
	...
	static R_W_NetInfo InitSetNetInfo[4] = {
		{(0x00000000 + (0x0009 << 8) + (0x00 << 3)) , 6, DefMac},
		{(0x00000000 + (0x0001 << 8) + (0x00 << 3)), 4, DefGw},
		{(0x00000000 + (0x0005 << 8) + (0x00 << 3)), 4, DefSn},
		{(0x00000000 + (0x000F << 8) + (0x00 << 3)), 4, DefIp}
	};
	static R_W_NetInfo InitReadNetInfo[4] ={
		{(0x00000000 + (0x0009 << 8) + (0x00 << 3)) , 6, TmpNetInfo.mac},
		{(0x00000000 + (0x0001 << 8) + (0x00 << 3)), 4, TmpNetInfo.gw},
		{(0x00000000 + (0x0005 << 8) + (0x00 << 3)), 4, TmpNetInfo.sn},
		{(0x00000000 + (0x000F << 8) + (0x00 << 3)), 4, TmpNetInfo.ip}
	}; 
	uint8_t Tmp;
	static uint8_t SocketNum = 0;
	static uint8_t ms_Count = 0;
	static uint8_t Step_RW_NetInfo = 0;
	static int8_t smpResult = ((0x0) - 0);
	_Bool CommEndFlag;
	
	if(gu8W5500_SPI_Status == W5500_SPI_Idle){	
		switch(Step){
			case Init_HW_Reset_1:
				HAL_GPIO_WritePin(((GPIO_TypeDef *) (((0x40000000UL) + 0x08000000UL) + 0x1000UL)), ((uint16_t)0x0002), GPIO_PIN_RESET);
				ms_Count = 0;
			...
			case Init_HW_Reset_2:
				...
				if(ms_Count == 50){
					ms_Count = 0;
					HAL_GPIO_WritePin(((GPIO_TypeDef *) (((0x40000000UL) + 0x08000000UL) + 0x1000UL)), ((uint16_t)0x0002), GPIO_PIN_SET);
					gu8_W5500_Init_Steps = Init_SW_Reset;
				}
				break;
			case Init_SW_Reset:
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x0000 << 8) + (0x00 << 3)), 0x80);
			...
			case Init_PHY_Conf:
				...
				PHYconf.speed = 1;
				PHYconf.duplex = 1;
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x002E << 8) + (0x00 << 3)), w5500_phyconf_transfer_to_uint8(PHYconf));
				gu8_W5500_Init_Steps = Init_PHY_Read;			
				break;
			case Init_PHY_Read:				
				smp_w5500_spiDMA_read_byte((0x00000000 + (0x002E << 8) + (0x00 << 3)));
				gu8_W5500_Init_Steps = Init_PHY_Rst;				
				break;
			case Init_PHY_Rst:				
				Tmp = Readbyte_value;
				Tmp |= ~~(1<<7);
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x002E << 8) + (0x00 << 3)), Tmp);
			...
			case Init_Verify_NetInfo:
				...
				memcpy(TmpNetInfo.dns ,DefDns,  4);
				if( (memcmp(TmpNetInfo.mac, DefMac, 6)!=0) ||
					(memcmp(TmpNetInfo.ip, DefIp, 4)!=0) ||
					(memcmp(TmpNetInfo.sn, DefSn, 4)!=0) ||
					(memcmp(TmpNetInfo.gw, DefGw, 4)!=0)){
					smpResult = ((0x0) - 1);
				}
				gu8_W5500_Init_Steps = Init_INT_MASK_Config;
				break;
			case Init_INT_MASK_Config:
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x0016 << 8) + (0x00 << 3)),0xFF);
				gu8_W5500_Init_Steps = Init_SocketNum_INT_MASK_Config;
				break;
			case Init_SocketNum_INT_MASK_Config:
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x0018 << 8) + (0x00 << 3)),0xFF);
				gu8_W5500_Init_Steps = Init_SocketEvent_INT_MASK_Config;
				break;
			case Init_SocketEvent_INT_MASK_Config:
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x002C << 8) + ((1+4*SocketNum) << 3)), (0x08|0x04|0x02|0x01));
				SocketNum++;
				if(SocketNum == 8){
	...
	if(gu8W5500_SPI_Status == W5500_SPI_Idle){	
		switch(Step){
			case Server_Read_Socket_Status:
				Socket_Event_Handling_Status[SocketNum] = Event_Handle_Ing;
				smp_w5500_spiDMA_read_byte((0x00000000 + (0x0003 << 8) + ((1+4*SocketNum) << 3)));
			...
			case Set_Socket_Close:
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x0001 << 8) + ((1+4*SocketNum) << 3)), 0x10);
				gu8Socket_Sever_Step[SocketNum] = Read_Cmd_Register_Status_0;
				break;
			case Read_Cmd_Register_Status_0:
				smp_w5500_spiDMA_read_byte((0x00000000 + (0x0001 << 8) + ((1+4*SocketNum) << 3)));
			...
			case RST_All_INT_Flag:
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x0002 << 8) + ((1+4*SocketNum) << 3)), 0xFF&0x1F);
			...
			case Set_Socket_Mode:
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x0000 << 8) + ((1+4*SocketNum) << 3)), TempSocket->Parm.Protocol);
			...
			case Set_Socket_Port_Highbyte:
				if(!TempSocket->Parm.PortNum)
				{
					...
					smp_w5500_spiDMA_write_byte((0x00000000 + (0x0004 << 8) + ((1+4*SocketNum) << 3)), lu8TTemp);
			...
			case Set_Socket_Port_Lowbyte:
					...
					smp_w5500_spiDMA_write_byte(((0x00000000 + (0x0004 << 8) + ((1+4*SocketNum) << 3)) + (1<<8)), lu8TTemp);
					gu8Socket_Sever_Step[SocketNum] = Set_Socket_Open;
				break;
			case Set_Socket_Open:
					smp_w5500_spiDMA_write_byte((0x00000000 + (0x0001 << 8) + ((1+4*SocketNum) << 3)), 0x01);
			...
			case Check_Open_Cmd_Recv:
				if(Readbyte_value!=0){
					...
					sock_io_mode &= ~(1 <<SocketNum);
					sock_io_mode |= ((0x00 & 0x01) << SocketNum);   
					sock_is_sending &= ~(1<<SocketNum);
					sock_remained_size[SocketNum] = 0;
					sock_pack_info[SocketNum] = 0x00;
			...
			case Socket_Listen_Read_Mode:
				smp_w5500_spiDMA_read_byte((0x00000000 + (0x0000 << 8) + ((1+4*SocketNum) << 3)));
			...
			case Set_Socket_Listen:
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x0001 << 8) + ((1+4*SocketNum) << 3)), 0x02);
			...
			case Clear_CON_INT_Flag:
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x0002 << 8) + ((1+4*SocketNum) << 3)), (0x01&0x1F)); 
				gu8Socket_Sever_Step[SocketNum] = Read_Socket_INT_event;
				break;
			case Read_Remote_IP:
				smp_w5500_spiDMA_ReadMulti((0x00000000 + (0x000C << 8) + ((1+4*SocketNum) << 3)),4, RemoteNetInfo.ip);
				gu8Socket_Sever_Step[SocketNum] =  Read_Remote_Port_HighByte;
				break;
			case Read_Remote_Port_HighByte:
				smp_w5500_spiDMA_read_byte((0x00000000 + (0x0010 << 8) + ((1+4*SocketNum) << 3)));
			...
			case Save_High_Byte_Read_Remote_Port_LowByte:
				...
				smp_w5500_spiDMA_read_byte(((0x00000000 + (0x0010 << 8) + ((1+4*SocketNum) << 3)) + (1<<8)));
			...
			case Read_Recv_Cnt_HighByte:
				smp_w5500_spiDMA_read_byte((0x00000000 + (0x0026 << 8) + ((1+4*SocketNum) << 3)));
			...
			case Read_Recv_Cnt_LowByte:
				...
				smp_w5500_spiDMA_read_byte(((0x00000000 + (0x0026 << 8) + ((1+4*SocketNum) << 3)) + (1<<8)));
			...
			case Read_Buf_Max_Len:
					smp_w5500_spiDMA_read_byte((0x00000000 + (0x001E << 8) + ((1+4*SocketNum) << 3)));
			...
			case ParserData_and_Update_Buf_Offset_Highbyte:
				...
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x0028 << 8) + ((1+4*SocketNum) << 3)),(uint8_t)(gu16Temp>>8));
				gu8Socket_Sever_Step[SocketNum] = Update_Buf_Offset_Lowbyte;
				break;
			case Update_Buf_Offset_Lowbyte:
				smp_w5500_spiDMA_write_byte(((0x00000000 + (0x0028 << 8) + ((1+4*SocketNum) << 3)) + (1<<8)),(uint8_t)gu16Temp);
				gu8Socket_Sever_Step[SocketNum] = Set_Scoket_Recv;
				break;
			case Set_Scoket_Recv:
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x0001 << 8) + ((1+4*SocketNum) << 3)), 0x40);
			...
			case Read_Tx_Buf_Max_Len:
				smp_w5500_spiDMA_read_byte((0x00000000 + (0x001F << 8) + ((1+4*SocketNum) << 3)));
			...
			case Update_Tx_Ptr_Highbyte:
				...
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x0024 << 8) + ((1+4*SocketNum) << 3)),(uint8_t)(gu16Temp>>8));
				gu8Socket_Sever_Step[SocketNum] = Update_Tx_Ptr_Lowbyte;
				break;
			case Update_Tx_Ptr_Lowbyte:
				smp_w5500_spiDMA_write_byte(((0x00000000 + (0x0024 << 8) + ((1+4*SocketNum) << 3)) + (1<<8)),(uint8_t)gu16Temp);
				gu8Socket_Sever_Step[SocketNum] = Set_Socket_Send;
				break;
			case Set_Socket_Send:
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x0001 << 8) + ((1+4*SocketNum) << 3)), 0x20);
			...
			case Clear_SendOK_INT_Flag:
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x0002 << 8) + ((1+4*SocketNum) << 3)), (0x04 &0x1F)); 
				gu8Socket_Sever_Step[SocketNum] = Read_Socket_INT_event;
			break;
			 
			 	
			case Set_Socket_Disconnect:
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x0001 << 8) + ((1+4*SocketNum) << 3)), 0x08);
			...
			case Clear_DISCON_INT_Flag:
				smp_w5500_spiDMA_write_byte((0x00000000 + (0x0002 << 8) + ((1+4*SocketNum) << 3)), (0x02 &0x1F)); 
				gu8Socket_Sever_Step[SocketNum] = Server_Read_Socket_Status;
				break;
