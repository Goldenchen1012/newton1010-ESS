...
{
	HAL_RTCEx_BKUPWrite(&RtcHandle, 0x00u, MagicCode);
}
uint16_t apiRamLoadRtcMagicCode(void)
{
	return HAL_RTCEx_BKUPRead(&RtcHandle, 0x00u);
}
...
void apiRamSaveSoc(uint16_t soc)
{
	...
	RamData.i[0] = soc;
	RamData.i[1] = RamData.i[0] ^ 0xAA55;
	HAL_RTCEx_BKUPWrite(&RtcHandle, 0x01u, RamData.l);
}
...
{	
	...
	RamData.l = HAL_RTCEx_BKUPRead(&RtcHandle, 0x01u);
	checksum = RamData.i[1];
	checksum ^= RamData.i[0];
	if(checksum == 0xAA55 && RamData.i[0] <= 10000)
		return RamData.i[0];
	return 0xffff;
}
...
{
	uint32_t	checksum;
	checksum = count ^ 0x937A;
	HAL_RTCEx_BKUPWrite(&RtcHandle, 0x06u, count);
	HAL_RTCEx_BKUPWrite(&RtcHandle, 0x07u, checksum);
}
uint32_t apiRamLoadTotalDisChargeCount(void)
{
	uint32_t	count, checksum;
	
	count = HAL_RTCEx_BKUPRead(&RtcHandle, 0x06u);
	checksum = HAL_RTCEx_BKUPRead(&RtcHandle, 0x07u);
...
#line 109 "..\\..\\..\\API\\ApiRamData.c"
void ApiRamSaveAccPower(void)
{
#line 137 "..\\..\\..\\API\\ApiRamData.c"
}
void ApiRamLoadAccPower(void)
{
#line 195 "..\\..\\..\\API\\ApiRamData.c"
}
...
void apiRamSaveRtcDateTime(void)
{
	...
	checksum = sec ^ 0x551384AAL;
	HAL_RTCEx_BKUPWrite(&RtcHandle, 0x04u, sec);
	HAL_RTCEx_BKUPWrite(&RtcHandle, 0x05u, checksum);
}
uint32_t apiRamLoadPowerOffDateTime(void)
{
	uint32_t	sec,checksum;
	
	sec = HAL_RTCEx_BKUPRead(&RtcHandle, 0x04u);
	checksum = HAL_RTCEx_BKUPRead(&RtcHandle, 0x05u);
	checksum ^= sec; 
	if(checksum == 0x551384AAL)
		return sec;
	return 0xffffffffL;
}
...
void apiRamSaveLastChgDhgTime(void)
{
	uint32_t	sec,checksum;
	sec = HalRtcGetSmpUnixTime();
	checksum = sec ^ 0xAA551384L;
	HAL_RTCEx_BKUPWrite(&RtcHandle, 0x02u, sec);
	HAL_RTCEx_BKUPWrite(&RtcHandle, 0x03u, checksum);
}	
...
uint32_t apiRamLoadReleaseTime(void)
{
	...
	sec = HAL_RTCEx_BKUPRead(&RtcHandle, 0x02u);
	checksum = HAL_RTCEx_BKUPRead(&RtcHandle, 0x03u);

	checksum ^= sec;

	if(checksum == 0xAA551384L)
	{
