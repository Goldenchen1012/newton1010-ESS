...
static uint8_t	BalanceFlag[(400 / 8) + 1]={0};
static uint8_t	BalanceFlagTemp[(400 / 8) + 1]={0};
...
static tLibRegisterEvtHandler EvtHandler;
...
static uint8_t getPhysicalCellPosition(uint16_t CellIndex, uint8_t *BmuIndex,uint8_t *Position)
{
	...
	for(bmu=0; bmu<apiSysParGetBmuNumber(); bmu++)
	{
...
static void setupPhysicalBalancePosition(void)
{
	...
	B = 0;
	for(cell=0; cell<apiSysParGetCellNumber(); B++)
	{
		for(b=0; b<8; b++,cell++)
		{
			if(cell >= apiSysParGetCellNumber())
				break;
...
static void appBalanceCheckEvent(void)
{
	...
	B = 0;
	for(cell=0; cell<apiSysParGetCellNumber(); cell+=8,B++)
	{  
		...
		if(diff)
		{
			...
			for(b=0; b<8; b++,index++)
			{
				if(index >= apiSysParGetCellNumber())
					break;
...
{	
	...
	else if(appGaugeGetCurrentMode() == 0)
	{
...
static void appBalanceSwTimerHandler( void *dest, uint16_t evt, void *vDataPtr)
{
...
void appBalanceOpen(tLibRegisterEvtHandler evtHandler)
{  	
