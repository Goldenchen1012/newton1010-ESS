...
static uint8_t IRMonitoring_Init(uint8_t exe_interval_s, uint16_t sw_delay_ms, apiIRMonitoring_cb_t callbackfunc)
{
    ...
    if(callbackfunc.DataReady_cb==0){
		  return(0);
		}	
	
    if(callbackfunc.TriggerData_cb==0){
		   return(0);
		}else{
		   irm_event_cb.TriggerData_cb = callbackfunc.TriggerData_cb;
		}			
		
	  if(callbackfunc.GetVoltDeviceInit_cb==0){
	      return(0);
	  }else{
    ...
    if(callbackfunc.irm_outdata==0){
        return(0);
		}else{   
		    irm_event_cb.irm_outdata = callbackfunc.irm_outdata;
		}
    
    if(callbackfunc.SW_gpioinit_cb==0){
        return(0);
		}else{   
    ...
    for(int i=0; i<3 ; i++){		
        if(callbackfunc.SW_gpio_crtl_cb[i]==0){
            return(0);
		    }else{   
	  ...
	  if(irm_event_cb.SW_gpioinit_cb!=0){
...
static void IRMonitoring_CtrlSW(IRMonitoring_SW_enum sw1, IRMonitoring_SW_enum sw2, IRMonitoring_SW_enum sw3){
	  ...
	  if(irm_event_cb.SW_gpio_crtl_cb[0]!=0){
	      if(sw1 == IRM_SW_ON){
	  ...
	  if(irm_event_cb.SW_gpio_crtl_cb[1]!=0){
	      if(sw2 == IRM_SW_ON){
	  ...
	  if(irm_event_cb.SW_gpio_crtl_cb[2]!=0){
	      if(sw3 == IRM_SW_ON){
...
static uint16_t  IRM_Balance_formula(IRMonitoring_Data_t data, IRMonitoring_Resistor_t *res_out){
	...
	ra = ((float)800000.0f)*1.0f;
	rb = ((float)800000.0f)*1.0f;
	rc = ((float)2200.0f)*1.0f;
    ...
    switch(steps){
			...
			case IRM_S2:
				  if(irm_data_ready_f == 1){
			        ...
			        irm_data.V_stack  = (irm_data.Vo_stack * ((800000.0f + 800000.0f + 2200.0f + 0.0f)/2200.0f));
			
              
	            if((irm_event_cb.irm_outdata != 0) && (irm_vstack_f==1)){	
			...
			case IRM_S3:
				  if(irm_data_ready_f == 1){
						 ...
						 irm_data.Vn   = (irm_data.Vo_n * ((800000.0f + 2200.0f + 0.0f)/2200.0f));  					
					}
          ...
          irm_data.Vp = (irm_data.V_stack - irm_data.Vn);  
		      ...
		      break;
			case IRM_S4:			
			    if(((fabs(irm_data.Vp - irm_data.Vn))/irm_data.V_stack) < 1.0f){
					    irm_sub_step = 1;
					    irm_mes_step = IRM_S5;
					}else{
					    irm_sub_step = 1;
		      ...
		      switch(irm_sub_step){
                ...
                irm_sub_step++;						
				        ...
				        if(irm_data_ready_f == 1){
						        ...
						        irm_data.Vn_l   = (irm_data.Vo_n_l * ((800000.0f + 2200.0f + 0.0f)/2200.0f));  								
								}	
...
#line 304 "..\\..\\..\\API\\ApiIRMonitoring.c"
		      ...
		      switch(irm_sub_step){
                ...
                irm_sub_step++;							     
				        ...
				        if(irm_data_ready_f == 1){
						        ...
						        irm_data.Vn   =(irm_data.Vo_en*((((800000.0f + 2200.0f + 0.0f)/2200.0f)*(1+500000.0f/800000.0f))+(500000.0f/2200.0f)))-((500000.0f/800000.0f)*irm_data.V_stack);
								}							
                ...
                irm_sub_step++;
						...
						case 4:
						        ...
						        irm_data_ready_f = 0;	
						        irm_data.Vo_n_l = irm_adc_data;
						        irm_data.Vn_l   = (irm_data.Vo_n_l * ((800000.0f + 2200.0f + 500000.0f + 0.0f)/2200.0f));   
								}
...
#line 364 "..\\..\\..\\API\\ApiIRMonitoring.c"
          ...
          ++sub_step_count;
					if(sub_step_count>10){
					    sub_step_count = 0;
              return(IRM_STEP_READ_TIMEOUT);
					}			
					
				  if(irm_event_cb.TriggerData_cb !=0){
...
static void IRMonitoringSwTimerHandler( void *dest, uint16_t evt, void *vDataPtr)
{
