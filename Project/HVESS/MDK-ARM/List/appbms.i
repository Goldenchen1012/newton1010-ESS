...
#line 29 "..\\..\\..\\APP\\AppBms.c"
#line 30 "..\\..\\..\\APP\\AppBms.c"
#line 1 "..\\..\\..\\APP\\AppBms.h"
...
static void assignSlaveIdSwTimerHandler( void *dest, uint16_t evt, void *vDataPtr);
static void masterSwTimerHandler( void *dest, uint16_t evt, void *vDataPtr);
...
{
	if((pBmsBuf->DataValidFlag & 0x0007) == 0x0007)
		return 1;
	else
		return 0;
}
...
{
	if(pBmsBuf->SystemFlag1 & ((1 << 2) | (1 << 3) | (1 << 6) | (1 << 7) | (1 << 10) | (1 << 13) | (1 << 16) | (1 << 19) | (1 << 20) | (1 << 21) | (1 << 24) | (1 << 25) | (1 << 28) | (1 << 29) | 0))
		return 1;
	if(pBmsBuf->SystemFlag2 & (0 | (1 << 3) | (1 << 4) | (1 << 5) | (1 << 6) | 0))
		return 1;
	return 0;
}
static void checkWhichScuRelayOn(void)
{
	...
	for(ScuIndex=0; ScuIndex<64; ScuIndex++)
	{
	...
	{		
		...
		if(MaxVbScuID != 0xff)	
		{
			...
			{
				CanPkg.id = (((uint32_t)0x02<<25)|((uint32_t)MaxVbScuID<<18)|((uint32_t)0x04<<10)|(uint32_t)ScuID);
	...
	else
	{
		...
		for(ScuIndex=0; ScuIndex<64; ScuIndex++)
		{
			...
			;
			if(diffv < apiSysParGetRelayOnDiffVoltage())	
			{
				if((ScuIndex + 1) == ScuID)
				{	
					apiRelayControlSetMasterTurnOnFlag();
				...
				{
					CanPkg.id = (((uint32_t)0x02<<25)|((uint32_t)ScuIndex + 1<<18)|((uint32_t)0x04<<10)|(uint32_t)ScuID);
...
static void outputNextScuCanRequestIdSignal(void)	
{
...
static void sendResetScuIdCanPackage(void)
{
	...
	CanPkg.id = (((uint32_t)0x04<<25)|((uint32_t)0<<18)|((uint32_t)0x00<<10)|(uint32_t)0);
...
static void sendRequestIdCanPackage(void)
{
	...
	CanPkg.id = (((uint32_t)0x04<<25)|((uint32_t)0<<18)|((uint32_t)0x01<<10)|(uint32_t)0);
...
static void checkOdInSwTimerHandler( void *dest, uint16_t evt, void *vDataPtr)
{
	char	str[100];
...
void appBmsSetScuSystemFlag(uint8_t scuid, uint32_t flag1, uint32_t flag2)
{
	...
	mBmsBuf[scuid-1].DataValidFlag |= 0x0004;
...
void appBmsSetScuCurrent(uint8_t scuid, int32_t CurrentP, int32_t CurrentN)
{
	...
	mBmsBuf[scuid-1].DataValidFlag |= 0x0002;	
	updateScuIdleCount(scuid);
...
void appBmsSetScuVbat(uint8_t scuid, uint32_t VbInt, uint32_t VbExt)
{
	...
	mBmsBuf[scuid-1].DataValidFlag |= 0x0001;
	updateScuIdleCount(scuid);
