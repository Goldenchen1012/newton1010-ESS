...
#line 23 "..\\..\\..\\HAL\\HalAfeBq796xx.c"
#line 24 "..\\..\\..\\HAL\\HalAfeBq796xx.c"
...
tLibRegisterEvtHandler afeEvtHandler;
...
#line 84 "..\\..\\..\\HAL\\HalAfeBq796xx.c"

bq796xx_init_default_t afe_load_data = { . ov_threshold =OV_4175MV, . ov_step_threshold =OV_STEP_100MV, . uv_threshold =UV_2800MV, . ot_threshold =OT_10_PCT, . ut_threshold =UT_80_PCT, . ov_enable =BQ_ENABLE, . uv_enable =BQ_ENABLE, . ot_enable =BQ_DISABLE, . ut_enable =BQ_DISABLE, . cb_cycle_time =CB_CYC_T_10S, };
...
static uint16_t	ReaAfeCycleCount10Ms = 50;
...
static void setNtcChannel(void)
{
	...
	if(ntcIoIndex == 0)
	{
		...
		drv_bq796xx_Set_AFE_GPIO_type(STACK, 0x00, test_gpio_HL, AFE_GPIO4,0);
	}
	else if(ntcIoIndex == 1)
	{
		test_gpio_HL = 5-((NtcChannelSelect & 0x02)>>1);
		drv_bq796xx_Set_AFE_GPIO_type(STACK, 0x00, test_gpio_HL, AFE_GPIO5,0);
	}
	else if(ntcIoIndex == 2)
	{	
		test_gpio_HL = 5-((NtcChannelSelect & 0x04)>>2);
		drv_bq796xx_Set_AFE_GPIO_type(STACK, 0x00, test_gpio_HL, AFE_GPIO6,0);
	}
	else if(ntcIoIndex == 3)
	{
		test_gpio_HL = 5-((NtcChannelSelect & 0x08)>>3);
		drv_bq796xx_Set_AFE_GPIO_type(STACK, 0x00, test_gpio_HL, AFE_GPIO7,0);
	}
...
{
	if(StackCountForIni[BridgeDirection] < apiSysParGetBmuNumber())
	{
		if(BridgeDirection == DIR_NORTH)
		{
			ntcChannelStartIndex += ntcChannelOffset;
			if(ntcChannelStartIndex >= 16)
				ntcChannelStartIndex -= 16;
			ntcChannelOffset = 0;
		}
	}
	else
...
static void sendOutBalanceDataLo(void)
{
	...
	for(bmuindex=0; bmuindex<apiSysParGetBmuNumber(); bmuindex++)
	{
		if(BridgeDirection == DIR_NORTH)
			RealBalanceIndex = BalanceBmuIndex;
		else
			RealBalanceIndex = apiSysParGetBmuNumber() - BalanceBmuIndex - 1;
...
static void finishAfeReadProcessor(void)
{
	...
	{	
		if((StackCountForIni[DIR_NORTH]+StackCountForIni[DIR_SOUTH]) <
			apiSysParGetBmuNumber())
		{
			fail_count++;
			if(run_count >= 6 && fail_count >= (6/2))
			{
#line 822 "..\\..\\..\\HAL\\HalAfeBq796xx.c"
			}
		}
		else
		{
		...
		{BridgeDirection ^= 0x01,((GPIO_TypeDef *) (((0x40000000UL) + 0x08000000UL) + 0x0C00UL))->ODR ^= ((uint16_t)0x2000);};
	...
	{		
		...
		if(RealPaserCount[BridgeDirection] != StackCountForIni[BridgeDirection])
		{
			...
			if(PaserCountFailCount[BridgeDirection] >= 6)
			{
...
#line 962 "..\\..\\..\\HAL\\HalAfeBq796xx.c"
		...
		;
		if((IsRingDaisyChain && res == apiSysParGetBmuNumber()) ||
		...
		if(cnt_delay <= 1)
		{
			...
			if(afe_steps > AFE_RUN_AUX_ADC)
			{
				...
				if((NorthAndSouthIniFlag&0x03) == 0x03 && 
					( (StackCountForIni[DIR_NORTH] == apiSysParGetBmuNumber()) ||
					  (StackCountForIni[DIR_SOUTH] == apiSysParGetBmuNumber()) ||
				...
				{					
					...
					if((NorthAndSouthIniFlag & 0x80) == 0)
					{
						...
						if(StackCountForIni[DIR_NORTH] == StackCountForIni[DIR_SOUTH] &&
						   StackCountForIni[DIR_NORTH] == (apiSysParGetBmuNumber() / 2))
						{
							;
							afe_steps = AFE_INIT_IDLE;
							cnt_delay = 50;
					...
					if(StackCountForIni[DIR_NORTH] == apiSysParGetBmuNumber() ||
					   StackCountForIni[DIR_SOUTH] == apiSysParGetBmuNumber())
					{
						ReaAfeCycleCount10Ms = 50;
					}
...
static void afeSwTimerHandler( void *dest, uint16_t evt, void *vDataPtr)
{
	char	str[100];
	if(evt == LIB_SW_TIMER_EVT_SW_1S)
	{
		...
		if(AfeCommTimeOutCount<200)
			AfeCommTimeOutCount++;
		if(AfeCommTimeOutCount >= 5)
		{
			 if((AfeCommFlag & 0x01) == 0)
			 {
			 	AfeCommFlag |= 0x01;
			 	if(afeEvtHandler)
		...
		if(AfeCommTimeOutCount >= 20)
		{
			if((AfeCommFlag & 0x02) == 0)
			{
			 	AfeCommFlag |= 0x02;
			 	if(afeEvtHandler)
...
static void setupAfe2ndOvValue(void)
{
	...
	uint16_t		setvalue;	
	apiSysParGetOvpPar(0x10, &ProtectPar);
...
static void setupAfe2ndUvValue(void)
{
	...
	tScuProtectPar	ProtectPar;
	uint16_t		setvalue;	
	apiSysParGetUvpPar(0x10, &ProtectPar);
...
void halafeOpen(tLibRegisterEvtHandler evtHandler)
{
	...
	AfeState = AFE_STATE_INI;
	AfeFailTimerCount  = 0;
	afe_load_data.bmu_total_num = apiSysParGetBmuNumber();
	setupAfe2ndOvValue();
	setupAfe2ndUvValue();
	setupAfe2ndOtValue();
	setupAfe2ndUtValue();
