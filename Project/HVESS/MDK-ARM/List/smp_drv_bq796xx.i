...
#line 20 "..\\..\\..\\Drivers\\SMP\\smp_drv_bq796xx.c"
...
#line 21 "..\\..\\..\\Drivers\\SMP\\smp_drv_bq796xx.c"
#line 22 "..\\..\\..\\Drivers\\SMP\\smp_drv_bq796xx.c"
...
#line 23 "..\\..\\..\\Drivers\\SMP\\smp_drv_bq796xx.c"
...
#line 24 "..\\..\\..\\Drivers\\SMP\\smp_drv_bq796xx.c"
#line 1 "..\\..\\..\\User\\main.h"
...
#line 25 "..\\..\\..\\Drivers\\SMP\\smp_drv_bq796xx.c"
#line 26 "..\\..\\..\\Drivers\\SMP\\smp_drv_bq796xx.c"
...
#line 27 "..\\..\\..\\Drivers\\SMP\\smp_drv_bq796xx.c"
...
#line 28 "..\\..\\..\\Drivers\\SMP\\smp_drv_bq796xx.c"
...
uint8_t  bq796xx_tx_buffer[512] = {0};
uint8_t  bq796xx_rx_buffer[(512*1)] = {0};
uint8_t  bq796xx_res_buf[(128+6)]={0};
uint16_t bq796xx_res_buf_c = 0;

#line 49 "..\\..\\..\\Drivers\\SMP\\smp_drv_bq796xx.c"

smp_gpio_t      bq796xx_gpio_ncs = { . port = SMP_GPIOA, . pin = PIN15, . mode = SMP_GPIO_MODE_OUTPUT_PP, . gpio_handler = 0 };
smp_gpio_state  bq796xx_gpio_ncs_state;    
                                
smp_gpio_t      bq796xx_gpio_rx  = { . port = SMP_GPIOC, . pin = PIN10, . mode = SMP_GPIO_MODE_OUTPUT_PP, . gpio_handler = 0 };
smp_gpio_state  bq796xx_gpio_rx_state;    
      

#line 70 "..\\..\\..\\Drivers\\SMP\\smp_drv_bq796xx.c"
													
bq796xx_init_default_t bq796xx_default={ . ov_threshold =OV_4175MV, . ov_step_threshold =OV_STEP_50MV, . uv_threshold =UV_3000MV, . ot_threshold =OT_10_PCT, . ut_threshold =UT_80_PCT, . ov_enable =BQ_ENABLE, . uv_enable =BQ_ENABLE, . ot_enable =BQ_ENABLE, . ut_enable =BQ_ENABLE, . cb_cycle_time =CB_CYC_T_10S, . bmu_total_num =13, };
	                   												
uint8_t bq796xx_afe_gpio_conf[4]={0};                          
...
smp_uart_t bq796xx_uart = { . num = UART0, . baud_rate = 1000000, . flow_ctrl = UART_FLOW_CTRL_DISABLE, . use_parity = PARITY_NONE, . buffers . rx_buf = bq796xx_rx_buffer, . buffers . rx_buf_size = (512*1), . buffers . tx_buf = bq796xx_tx_buffer, . buffers . tx_buf_size = 512, };
...
void drv_bq796xx_HwTimerHanlder( void *dest, uint16_t evt, void *vDataPtr)
{
	smp_time_count++;
}
...
void drv_bq796xx_switch_rx_pin_type_setting(bq796xx_io_type type){
    ...
    case BQ_UART:
        ...
        if(smp_uart_init(&bq796xx_uart, drv_bq796xx_uart_event_handler)==((0x0) - 0)){
            SEGGER_RTT_printf(0,"%s%s" "BMU UART initial success!\r\n" "%s", "\x1B[1;35m", "", "\x1B[0m");
        }else{
            SEGGER_RTT_printf(0,"%s%s" "BMU UART initial fail!\r\n" "%s", "\x1B[1;35m", "", "\x1B[0m");
        }  
        break;    
    }
}
...
void drv_bq796xx_clear_fifobuffer(void){
    ...
    while(smp_uart_get(&bq796xx_uart, &rx_data)==((0x0) - 0)){
        bq796xx_res_buf_c--;
    }
		((GPIO_TypeDef *) (((0x40000000UL) + 0x08000000UL) + 0x0C00UL))->ODR &= ~((uint16_t)0x4000);
}
...
uint8_t drv_bq796xx_start_setting(uint8_t maxcnt, uint8_t dir){
    ...
    fill_data4_payload(d_payload,0x20 | dir_sel,0,0,0);
    drv_bq796xx_command_framing(SINGLE_WRITE, 0x00, 0x0309, 1, d_payload, 1);
    drv_bq796xx_delay_ms(5*bq796xx_default.bmu_total_num);                                                                                            
    
    fill_data4_payload(d_payload,0x10+0x04,0,0,0);
    drv_bq796xx_command_framing(SINGLE_WRITE, 0x00, 0x2001, 1, d_payload, 1);                               
    drv_bq796xx_command_framing(SINGLE_WRITE, 0x00, 0x034C, 1, null_payload,   1);                       
    ...
    if ( dir == DIR_SOUTH ){
         ...
         drv_bq796xx_command_framing(BROAD_WRITE_REVERSE, 0, 0x0309, 1, d_payload, 1);     
    }else{
    ...
    drv_bq796xx_command_framing(BROAD_WRITE, 0x00, 0x034C, 1, null_payload, 1);                          
    
    
    
    
    for ( i = 0; i < 8; i++ ){                                                                                                            
      drv_bq796xx_command_framing(BROAD_WRITE, 0, 0x0343+i, 1, null_payload, 1);
    }
    ...
    fill_data4_payload(d_payload,1 | dir_sel,0,0,0);
    drv_bq796xx_command_framing(BROAD_WRITE, 0, 0x0309, 1, d_payload, 1);

    
    for ( i = 0; i <= 0x3F; i++ ){                                                                                      
        fill_data4_payload(d_payload,i,0,0,0);
        drv_bq796xx_command_framing(BROAD_WRITE, 0, dir_addr, 1, d_payload, 1);                             
    }
    drv_bq796xx_delay_ms(10);
    
    
    for ( i = 0; i < 8; i++ ){                                                                                                 
        drv_bq796xx_command_framing(BROAD_READ, 0, 0x0343+i, 1, null_payload, 1);
    }
    drv_bq796xx_delay_ms(10);
    
    
    for ( bmu_cnt = 1; bmu_cnt <= top_no; bmu_cnt++){
        drv_bq796xx_command_framing(SINGLE_READ,  bmu_cnt, dir_addr, 1, null_payload, 1);                  
    ...
    drv_bq796xx_command_framing(BROAD_WRITE,   0, 0x0308, 1, d_payload, 1);                                 
    fill_data4_payload(d_payload,0,0,0,0);
    drv_bq796xx_command_framing(SINGLE_WRITE,  0x00, 0x0308, 1, d_payload, 1);                              
    if (bmu_cnt > maxcnt ){
        bmu_cnt = maxcnt;
    }
    
    fill_data4_payload(d_payload,3,0,0,0);
    drv_bq796xx_command_framing(SINGLE_WRITE,  bmu_cnt, 0x0308, 1, d_payload, 1);                          
    
        
    
    
    fill_data4_payload(d_payload,0xFF,0,0,0);
    drv_bq796xx_command_framing(BROAD_WRITE,   0, 0x0331, 1, d_payload, 1);                               
    drv_bq796xx_command_framing(BROAD_WRITE,   0, 0x0332, 1, d_payload, 1);                               
    
    fill_data4_payload(d_payload,0x20 | 0x02,0,0,0);
    drv_bq796xx_command_framing(SINGLE_WRITE,  0x00, 0x2030, 1,d_payload , 1);                             
    
    drv_bq796xx_command_framing(STACK_WRITE,   0, 0x0013, 1, null_payload, 1);                           
    drv_bq796xx_command_framing(SINGLE_WRITE,  0x00, 0x2005, 1, null_payload, 1);                       


    
    
    fill_data4_payload(d_payload,0x01,0,0,0);
    drv_bq796xx_command_framing(STACK_WRITE,   0, 0x030A, 1, d_payload, 1);                                  
    

    
    
    fill_data4_payload(d_payload,16-6,0,0,0);
    drv_bq796xx_command_framing(BROAD_WRITE,   0, 0x0003, 1, d_payload, 1);                              
    
    fill_data4_payload(d_payload,0x03,0,0,0);
    drv_bq796xx_command_framing(BROAD_WRITE,   0, 0x0007, 1, d_payload, 1);                                
    
    fill_data4_payload(d_payload,0x1E,0,0,0);
    drv_bq796xx_command_framing(STACK_WRITE,   0, 0x030D, 1, d_payload, 1);                                
    drv_bq796xx_delay_ms(38+5*bmu_cnt);                                                                                                  
    
    
    fill_data4_payload(d_payload,0x0D,0,0,0);
    drv_bq796xx_command_framing(STACK_WRITE,   0, 0x0019, 1, d_payload, 1);                         
    
    
		
		
		fill_data4_payload(d_payload,((0x10 | 0x04) &(~(0x40))), 0, 0, 0);   
    drv_bq796xx_command_framing(STACK_WRITE, 0x00, 0x0002,1, d_payload, 0);
...
uint8_t drv_bq796xx_Read_AFE_ALL_VCELL(bq796xx_AFE_GPIO_stack is_stack,uint8_t dev_id,uint32_t delays){
  ...
  fill_data4_payload(d_payload,(16*2)-1,0,0,0); 
  
  if(is_stack == STACK)  
      drv_bq796xx_command_framing(STACK_READ, 0, 0x0568, 1, d_payload, delays);
  else
      drv_bq796xx_command_framing(SINGLE_READ, dev_id, 0x0568, 1, d_payload, delays);
...
uint8_t drv_bq796xx_Set_AFE_GPIO_type(bq796xx_AFE_GPIO_stack is_stack,uint8_t dev_id,bq796xx_AFE_GPIO_Type GPIO_type,bq796xx_AFE_GPIO GPIO_Num,uint32_t delays){
  ...
  set_reg_addr = 0x000E +(GPIO_Num>>1);
...
uint8_t drv_bq796xx_Start_AFE_ADC(bq796xx_AFE_GPIO_stack is_stack,uint8_t dev_id,uint32_t delays){
  ...
  fill_data4_payload(adc_ctrl_1_d,(0x10 | 0x08 | 0x04 | 0x02),0x00,(0x04 | 0x02),0);
  
  if(is_stack == STACK){  
      drv_bq796xx_command_framing(STACK_WRITE, 0, 0x030D, 3, adc_ctrl_1_d, delays);
  }else{
      drv_bq796xx_command_framing(SINGLE_WRITE, dev_id, 0x030D, 3, adc_ctrl_1_d, delays);
  }
...
uint8_t drv_bq796xx_Stop_AFE_ADC(bq796xx_AFE_GPIO_stack is_stack,uint8_t dev_id,uint32_t delays){
  ...
  fill_data4_payload(adc_ctrl_1_d,(0x10 | 0x08 | 0x02), 0x00, (0x02),0);
...
uint8_t drv_bq796xx_Read_AFE_ALL_ADC(bq796xx_AFE_GPIO_stack is_stack,uint8_t dev_id,uint32_t delays){
  ...
  fill_data4_payload(d_payload,(8*2)-1,0,0,0); 
  
  if(is_stack == STACK)  
      drv_bq796xx_command_framing(STACK_READ, 0, 0x058E, 1, d_payload, delays);
  else
      drv_bq796xx_command_framing(SINGLE_READ, dev_id, 0x058E, 1, d_payload, delays);
...
uint8_t drv_bq796xx_Goto_ShutDownMode(uint32_t delays){
  ...
  fill_data4_payload(d_payload,0x08,0,0,0); 
  drv_bq796xx_command_framing(BROAD_WRITE, 0x00, 0x0309, 1, d_payload, delays);
...
uint8_t drv_bq796xx_Goto_SleepMode(uint32_t delays){
  ...
  fill_data4_payload(d_payload,0x04,0,0,0); 
  drv_bq796xx_command_framing(SINGLE_WRITE, 0x00, 0x0309, 1, d_payload, delays);
...
uint8_t drv_bq796xx_Send_WakeupAll(uint32_t delays){
  ...
  drv_bq796xx_switch_rx_pin_type_setting(BQ_UART);
  drv_bq796xx_delay_ms(30);
  fill_data4_payload(d_payload,0x20,0,0,0); 
  drv_bq796xx_command_framing(SINGLE_WRITE, 0x00, 0x0309, 1, d_payload, delays);
...
uint8_t drv_bq796xx_Set_Mask_FaultAll(bq796xx_AFE_GPIO_stack is_stack, uint8_t dev_id, uint32_t delays){
  ...
  fill_data4_payload(d_payload,0xFF,0xFF,0,0); 
	
	if(is_stack == STACK){
      drv_bq796xx_command_framing(STACK_WRITE, 0, 0x0016, 2, d_payload, delays);

		  bq796xx_afe_fault_msk1=0xFF;
		  bq796xx_afe_fault_msk2=0xFF;
  }else{
      drv_bq796xx_command_framing(SINGLE_WRITE, dev_id, 0x0016, 2, d_payload, delays);
	}
...
uint8_t drv_bq796xx_Clear_Mask_Fault_OVUV(bq796xx_AFE_GPIO_stack is_stack, uint8_t dev_id, uint32_t delays){
  static uint8_t    d_payload[4] = {0};
	...
	bq796xx_afe_fault_msk1 &= ~(0x08|0x10);
	
  fill_data4_payload(d_payload,bq796xx_afe_fault_msk1,0,0,0); 
  
	if(is_stack == STACK){
	    drv_bq796xx_command_framing(STACK_WRITE, 0, 0x0016, 1, d_payload, delays);
		  
	}else{
	    drv_bq796xx_command_framing(SINGLE_WRITE, dev_id, 0x0016, 1, d_payload, delays);
	}
...
uint8_t drv_bq796xx_Clear_Mask_Fault_OTUT(bq796xx_AFE_GPIO_stack is_stack, uint8_t dev_id, uint32_t delays){
  static uint8_t    d_payload[4] = {0};
	...
	bq796xx_afe_fault_msk1 &= ~(0x20|0x40);
	
  fill_data4_payload(d_payload,bq796xx_afe_fault_msk1,0,0,0); 
	
	if(is_stack == STACK){
      drv_bq796xx_command_framing(STACK_WRITE, dev_id, 0x0016, 1, d_payload, delays);
	}else{
      drv_bq796xx_command_framing(SINGLE_WRITE, dev_id, 0x0016, 1, d_payload, delays);
	}
...
uint8_t drv_bq796xx_Set_BroadCast_Mask_FaultAll(uint32_t delays){
  ...
  fill_data4_payload(d_payload,0xFF,0xFF,0,0); 
  drv_bq796xx_command_framing(BROAD_WRITE, 0x00, 0x0016, 2, d_payload, delays);
...
uint8_t drv_bq796xx_Set_BroadCast_Mask_FaultSel(uint8_t msk1,uint8_t msk2,uint32_t delays){
  ...
  fill_data4_payload(d_payload,msk1,msk2,0,0); 
  drv_bq796xx_command_framing(BROAD_WRITE, 0x00, 0x0016, 2, d_payload, delays);
...
uint8_t drv_bq796xx_Clear_FaultRstAll(uint32_t delays){
  ...
  fill_data4_payload(d_payload,0xFF,0xFF,0,0); 
  drv_bq796xx_command_framing(STACK_WRITE, 0x00, 0x0331, 2, d_payload, delays);
  drv_bq796xx_command_framing(SINGLE_WRITE, 0x00, 0x2030, 1, d_payload, delays);
...
uint8_t drv_bq796xx_Read_Stack_FaultSummary(uint32_t delays){
  ...
  fill_data4_payload(d_payload,0,0,0,0);
  drv_bq796xx_command_framing(STACK_READ, 0x00, 0x052D,1, d_payload, delays);
...
uint8_t drv_bq796xx_Read_Base_FaultSummary(uint32_t delays){
  ...
  fill_data4_payload(d_payload,0,0,0,0);
  drv_bq796xx_command_framing(SINGLE_READ, 0x00, 0x2100,1, d_payload, delays);
...
uint8_t drv_bq796xx_Set_Stack_OV(bq796xx_ov_range ov_range_mv,bq796xx_ov_step ov_step_mv ,uint32_t delays){
  ...
  drv_bq796xx_command_framing(STACK_WRITE, 0x00, 0x0009,1, d_payload, delays);
...
uint8_t drv_bq796xx_Set_Stack_UV(bq796xx_uv_mv uv_mv,uint32_t delays){
  ...
  drv_bq796xx_command_framing(STACK_WRITE, 0x00, 0x000A,1, d_payload, delays);
...
uint8_t drv_bq796xx_Run_OVUV(uint32_t delays){
  ...
  fill_data4_payload(d_payload,0x04 | 0x01,0,0,0);   
  drv_bq796xx_command_framing(STACK_WRITE, 0x00, 0x032C,1, d_payload, delays);
...
uint8_t drv_bq796xx_Read_Stack_FaultOVUV(uint32_t delays){
  ...
  fill_data4_payload(d_payload,3,0,0,0);    
  drv_bq796xx_command_framing(STACK_READ, 0x00, 0x053C,1, d_payload, delays);
...
uint8_t drv_bq796xx_Set_Stack_OTUT(bq796xx_ot_threshold_p ot_th_pct,bq796xx_ut_threshold_p ut_th_pct,uint32_t delays){
  ...
  drv_bq796xx_command_framing(STACK_WRITE, 0x00, 0x000B,1, d_payload, delays);
...
uint8_t drv_bq796xx_Run_OTUT(uint32_t delays){
  ...
  fill_data4_payload(d_payload,0x04 | 0x01,0,0,0);   
  drv_bq796xx_command_framing(STACK_WRITE, 0x00, 0x032D,1, d_payload, delays);
...
uint8_t drv_bq796xx_Read_Stack_FaultOTUT(uint32_t delays){
  ...
  drv_bq796xx_command_framing(STACK_READ, 0x00, 0x0540,1, d_payload, delays);
...
uint8_t drv_bq796xx_Set_Stack_CellBalanceTime(bq796xx_cellbalance_time cb_time,uint32_t delays){
  ...
  while(cell_cnt < 16){
     ...
     drv_bq796xx_command_framing(STACK_WRITE, 0x00, 0x0327-cell_cnt,1, d_payload, delays);
     ++cell_cnt;
  }
...
uint8_t drv_bq796xx_Set_Stack_CellBalanceCycleTime(bq796xx_cellbalance_cycle_time cb_cyc_time,uint32_t delays){
  ...
  drv_bq796xx_command_framing(STACK_WRITE, 0x00, 0x032E,1, d_payload, delays);
  return 0;
}
...
uint8_t drv_bq796xx_Stack_CellBalanceStarting(bq796xx_cellbalance_control cb_ctrl, uint32_t delays){
  static uint8_t    d_payload[4] = {0};  
	...
	if(cb_ctrl==CB_AUTO){
      fill_data4_payload(d_payload, (0x02 | 0x01),0,0,0);
	}else{
      fill_data4_payload(d_payload, (0x02),0,0,0);	
	}
	
  drv_bq796xx_command_framing(STACK_WRITE, 0x00, 0x032F,1, d_payload, delays);
...
uint8_t drv_bq796xx_check_respone_event(void){
  ...
  do{
    ...
    if(bq796xx_res_buf_c >= 5){ 
        rcv_temp_cnt = bq796xx_res_buf_c;
        break;
    }   
    if(smp_time_count_get() >= 5) return(0x00);      
  }while(1);
  
  smp_time_count_set(0);
  do{  
    if(bq796xx_res_buf_c > rcv_temp_cnt){ 
        smp_time_count_set(0);
    }else{
				rcv_temp_cnt = bq796xx_res_buf_c;
		}   
...
#line 918 "..\\..\\..\\Drivers\\SMP\\smp_drv_bq796xx.c"
	
  if(smp_uart_get(&bq796xx_uart, &rx_data)==((0x0) - 0)){
    --bq796xx_res_buf_c;
    if(rx_data < 128){
        bq796xx_res_buf[0] = rx_data;
        data_len=rx_data;        
  ...
  crc = bq796xx_res_buf[0+data_len+5] + bq796xx_res_buf[0+data_len+6]*256; 
  ...
  read_reg_adr = bq796xx_res_buf[2]*256+ bq796xx_res_buf[3];
  
	device_id = bq796xx_res_buf[1];
  ...
  switch(read_reg_adr){
    case 0x0568:
      for( i=0 ;i < (data_len+1)/2; i++){
          bq796xx_data.vcell_data[device_id-1][15-i]=bq796xx_res_buf[4+(i*2)] * 256.0f + bq796xx_res_buf[4+(i*2+1)];
      }
    ...
    case 0x058E:  
      for( i=0 ;i < (data_len+1)/2; i++){
          bq796xx_data.gpio_data[device_id-1][i]=bq796xx_res_buf[4+(i*2)] * 256.0f + bq796xx_res_buf[4+(i*2+1)];
      }    
      ...
      break;
    case 0x052D:  
      bq796xx_data.fault_summary[device_id-1] = bq796xx_res_buf[4];
		  
		  bq_event_type = BQ_EVENT_FAULT;
		
      break;
    case 0x053C:  
      bq796xx_data.fault_ov[device_id-1] = bq796xx_res_buf[4]*256+ bq796xx_res_buf[4+1];
      bq796xx_data.fault_uv[device_id-1] = bq796xx_res_buf[4+2]*256+ bq796xx_res_buf[4+3];
    
      bq_event_type = BQ_EVENT_FAULTOVUV;		
		
		  break;      
    case 0x0540:  
      bq796xx_data.fault_ot[device_id-1] = bq796xx_res_buf[4];
      bq796xx_data.fault_ut[device_id-1] = bq796xx_res_buf[4+1];
		
		  bq_event_type = BQ_EVENT_FAULTOTUT;		
      break;    
    case 0x0306:  
      bq796xx_data.top_stack_north_id = bq796xx_res_buf[4];
      bq796xx_data.top_stack_south_id = bq796xx_res_buf[4+1];
		
		  bq_event_type = BQ_EVENT_DIR_ADDR;		
      break; 
    case 0x0003:  
      bq796xx_data.ns_flag = (bq796xx_res_buf[4] & 0xC0);
...
uint8_t drv_bq796xx_Init_Steps(bq796xx_wake_tone_switch wake_tone_sw, bq796xx_init_steps_enum *afe_phase,uint8_t maxcnt, uint8_t dir, uint8_t *step_complete_f, uint8_t *before_delay_ms){
    ...
    switch(*afe_phase){
        ...
        case AFE_INIT_WAKE_UP:                                              
            ...
            break;
        case AFE_INIT_WAKE_UP_WAIT:                                                                                    
						*before_delay_ms = (10 * maxcnt);
						*step_complete_f = 1;						
            break;
        case AFE_INIT_BRORAD_SET_BMU_STACK1:                                                                           
				    fill_data4_payload(d_payload, 2,0,0,0);
            drv_bq796xx_command_framing(BROAD_WRITE, 0x00, 0x0308, 1, d_payload, 0);                        
            *before_delay_ms = 1;
            *step_complete_f = 1;
            break;		
        case AFE_INIT_SET_BASE1:
            fill_data4_payload(d_payload,0,0,0,0);
            drv_bq796xx_command_framing(SINGLE_WRITE,  0x00, 0x0308, 1, d_payload, 0); 
        ...
        case AFE_INIT_WAKE_DEV:                                             																					 
            ...
            drv_bq796xx_command_framing(SINGLE_WRITE, 0x00, 0x0309, 1, d_payload, 0);
        ...
        case AFE_INIT_FAULT_DET_ENABLE:
            fill_data4_payload(d_payload,0x10 | 0x04,0,0,0);
            drv_bq796xx_command_framing(SINGLE_WRITE, 0x00, 0x2001, 1, d_payload, 0);                       
        ...
        case AFE_INIT_OTP_ECC_TEST_DISABLE_BASE:
            drv_bq796xx_command_framing(SINGLE_WRITE, 0x00, 0x034C, 1, null_payload,   0); 
        ...
        case AFE_INIT_SET_DIR:
            fill_data4_payload(d_payload, dir_sel,0,0,0);                                                
            drv_bq796xx_command_framing(BROAD_WRITE_REVERSE, 0, 0x0309, 1, d_payload, 0);      
        ...
        case AFE_INIT_OTP_ECC_TEST_DISABLE_BROAD:
            drv_bq796xx_command_framing(BROAD_WRITE, 0x00, 0x034C, 1, null_payload, 0); 
				
						*before_delay_ms = 1;
						*step_complete_f = 1;
            break;
        case AFE_INIT_DUMMY_WRITE:					
            drv_bq796xx_command_framing(BROAD_WRITE, 0, 0x0343+sub_step, 1, null_payload, 0);                
            ++sub_step;
        ...
        case AFE_INIT_AUTO_ADDR:
            fill_data4_payload(d_payload,1 | dir_sel,0,0,0);                                                            
            drv_bq796xx_command_framing(BROAD_WRITE, 0, 0x0309, 1, d_payload, 0);
        ...
        case AFE_INIT_SET_ID:
            ...
            fill_data4_payload(d_payload,sub_step,0,0,0);                                                                     
            drv_bq796xx_command_framing(BROAD_WRITE, 0, dir_addr, 1, d_payload, 0);                                           
            ++sub_step;				
						*before_delay_ms = 1;
        ...
        case AFE_INIT_DUMMY_READ:         
            drv_bq796xx_command_framing(BROAD_READ, 0, 0x0343+sub_step, 1, null_payload, 0);                  
            ++sub_step;
        ...
        case AFE_INIT_BRORAD_SET_BMU_STACK2:
            ...
            *before_delay_ms = 1;                                                                                 
						if(rcv_check_step == 0){
						    bmu_cnt = sub_step;
                drv_bq796xx_clear_fifobuffer();
						    drv_bq796xx_command_framing(SINGLE_READ,  bmu_cnt, dir_addr, 1, null_payload, 0);                 
							  rcv_check_step = 1;
						}else{
        ...
        case AFE_INIT_SET_STACK:
            fill_data4_payload(d_payload,2,0,0,0);
            drv_bq796xx_command_framing(BROAD_WRITE,   0, 0x0308, 1, d_payload, 0);
        ...
        case AFE_INIT_SET_TOP:
            if (bmu_cnt > maxcnt ){
                bmu_cnt = maxcnt;
						...
						if(bmu_cnt>=1){
						    fill_data4_payload(d_payload,3,0,0,0);
                drv_bq796xx_command_framing(SINGLE_WRITE,  bmu_cnt, 0x0308, 1, d_payload, 0); 
						}
						*before_delay_ms = 1;
						*step_complete_f = 1;
            rcv_check_step = 0;						
            ...
            *before_delay_ms = 1;                                                                                   
						if(rcv_check_step == 0){
   			        fill_data4_payload(d_payload,0,0,0,0);
                drv_bq796xx_command_framing(SINGLE_READ,  bmu_cnt, 0x0003, 1, d_payload, 0);            
							  rcv_check_step = 1;
						}else{
                ...
                if(res == 0x01){
								    ++sub_step;
									  if ( dir == DIR_NORTH ){
										   if(bq796xx_data.ns_flag == 0x40){    
											     is_ring = 0x80;
											 }
										}else{
										   if(bq796xx_data.ns_flag == 0x80){    
											     is_ring = 0x80;
											 }											
										}
        ...
        case AFE_INIT_SET_DIRECTION_FLAG_ACTIVE_CELL:						
						if(bmu_cnt>= 0x01){
							  if(dir == DIR_NORTH )
								    fill_data4_payload(d_payload,(16-6) | 0x80,0,0,0);
								else
									  fill_data4_payload(d_payload,(16-6) | 0x40,0,0,0);
								drv_bq796xx_command_framing(STACK_WRITE, 0 , 0x0003, 1, d_payload, 0);
						}
        ...
        case AFE_INIT_FAULT_RESET_1:
            fill_data4_payload(d_payload,0xFF,0,0,0);
            drv_bq796xx_command_framing(BROAD_WRITE,   0, 0x0331, 1, d_payload, 0); 
						*before_delay_ms = 1;
						*step_complete_f = 1;	
            break;
        case AFE_INIT_FAULT_RESET_2:
            fill_data4_payload(d_payload,0xFF,0,0,0);
				    drv_bq796xx_command_framing(BROAD_WRITE,   0, 0x0332, 1, d_payload, 0); 
						*before_delay_ms = 1;
						*step_complete_f = 1;
            break;
        case AFE_INIT_FAULT_RESET:
            fill_data4_payload(d_payload,0x20 | 0x02,0,0,0);
				    drv_bq796xx_command_framing(SINGLE_WRITE,  0x00, 0x2030, 1,d_payload , 0);
						*before_delay_ms = 1;
						*step_complete_f = 1;
            break;
        case AFE_INIT_OTP_SPARE:
            drv_bq796xx_command_framing(STACK_WRITE,   0, 0x0013, 1, null_payload, 0);
						*before_delay_ms = 1;
						*step_complete_f = 1;
            break;
        case AFE_INIT_BQ79600_TIMEOUT:
            drv_bq796xx_command_framing(SINGLE_WRITE,  0x00, 0x2005, 1, null_payload, 0);
						*before_delay_ms = 1;
						*step_complete_f = 1;
            break;
        ...
        case AFE_INIT_TSREF_ENABLE:
            fill_data4_payload(d_payload,0x01,0,0,0);
            drv_bq796xx_command_framing(STACK_WRITE,   0, 0x030A, 1, d_payload, 0);
						*before_delay_ms = 1;
						*step_complete_f = 1;
     				break;
        case AFE_INIT_ADC_CONF1:
            fill_data4_payload(d_payload,0x03,0,0,0);
            drv_bq796xx_command_framing(BROAD_WRITE,   0, 0x0007, 1, d_payload, 0);
						*before_delay_ms = 1;
						*step_complete_f = 1;
            break;
        case AFE_INIT_ADC_CTRL1:
            fill_data4_payload(d_payload,0x1E,0,0,0);
            drv_bq796xx_command_framing(STACK_WRITE,   0, 0x030D, 1, d_payload, 0);                                

						*before_delay_ms = 38+5*bmu_cnt;
						*step_complete_f = 1;				
				    break;
        case AFE_INIT_BQ796XX_TIMEOUT:               
            fill_data4_payload(d_payload,0x0D,0,0,0);   
				    drv_bq796xx_command_framing(STACK_WRITE,   0, 0x0019, 1, d_payload, 0);
						*before_delay_ms = 1;
						*step_complete_f = 1;				
            break;
       case AFE_INIT_MASK_FAULT_ALL:      
            ...
            break;
			 case AFE_INIT_BRIDGE_FAULT_MSK:
  					fill_data4_payload(d_payload, (0x02 | 0x04| 0x10 | 0x40 | 0x80 | 0x08 | 0x01), 0, 0, 0);
						drv_bq796xx_command_framing(SINGLE_WRITE, 0x00, 0x2020, 1, d_payload, 0);
       ...
       case AFE_INIT_CLEAR_RST_ALL:
	          ...
	          switch(sub_step){
						    case 0:
			              drv_bq796xx_command_framing(STACK_WRITE, 0x00, 0x0331, 2, d_payload, 0);
                    break;
								case 1:
									  drv_bq796xx_command_framing(SINGLE_WRITE, 0x00, 0x2030, 1, d_payload, 0);
								    break;
            ...
            break;			
       case AFE_INIT_SET_RUN_OVUV_FUNC:   
				    *before_delay_ms = 1; 
            ...
            break;			 
       case AFE_INIT_SET_RUN_OTUT_FUNC:	
				    *before_delay_ms = 1; 
	          ...
	          switch(sub_step){				 
		       ...
		       break;				
			case AFE_INIT_SET_CELL_BALANCE_FUNC_DISABLE:  
				    *before_delay_ms = 1; 
			 
	          switch(sub_step){				 
						    case 0:				
                    fill_data4_payload(d_payload, 0,0,0,0); 
                    drv_bq796xx_command_framing(STACK_WRITE, 0x00, 0x0327-cell_cnt,1, d_payload, 0);
							      ++cell_cnt;
...
int8_t drv_bq796xx_init_default_load(bq796xx_init_default_t in_load_data){
	   ...
	   if(&(in_load_data)== 0) return ((0x0) - 11);
...
uint8_t drv_bq796xx_CellBalance_1to8_set(uint8_t bmu_id, uint8_t cell_d_en, bq796xx_cellbalance_time cb_time, uint32_t delays){
    ...
    static uint8_t payload_temp[8] = {0};
	  ...
	  drv_bq796xx_command_framing(SINGLE_WRITE, bmu_id, 0x0320, 8, payload_temp, delays);
...
uint8_t drv_bq796xx_CellBalance_9to16_set(uint8_t bmu_id, uint8_t cell_d_en, bq796xx_cellbalance_time cb_time, uint32_t delays){
    ...
    static uint8_t payload_temp[8] = {0};
	  ...
	  drv_bq796xx_command_framing(SINGLE_WRITE, bmu_id, 0x0318, 8, payload_temp, delays);
...
uint8_t drv_bq796xx_CellBalance_1to8_clear(bq796xx_AFE_GPIO_stack is_stack, uint8_t dev_id, uint32_t delays){
  static uint8_t    d_payload[8] = {0};  
	...
	if(is_stack == STACK)
      drv_bq796xx_command_framing(STACK_WRITE, 0x00, 0x0320, 8, d_payload, delays);
  else
		  drv_bq796xx_command_framing(SINGLE_WRITE, dev_id, 0x0320, 8, d_payload, delays);
...
uint8_t drv_bq796xx_CellBalance_9to16_clear(bq796xx_AFE_GPIO_stack is_stack, uint8_t dev_id, uint32_t delays){
  static uint8_t    d_payload[8] = {0};  
	...
	if(is_stack == STACK)
      drv_bq796xx_command_framing(STACK_WRITE, 0x00, 0x0318, 8, d_payload, delays);
  else
		  drv_bq796xx_command_framing(SINGLE_WRITE, dev_id, 0x0318, 8, d_payload, delays);
...
uint8_t drv_bq796xx_direction_set_steps(uint8_t ns_dir_bmu_cnt,bq796xx_dir_set_steps_enum *afe_phase,uint8_t maxcnt, uint8_t dir, uint8_t *step_complete_f, uint8_t *before_delay_ms){
    ...
    switch(*afe_phase){
        case SETDIR_BRORAD_SET_BMU_STACK1:                                                                             
				    fill_data4_payload(d_payload, 2,0,0,0);
            drv_bq796xx_command_framing(BROAD_WRITE, 0x00, 0x0308, 1, d_payload, 0);                        
            *before_delay_ms = 1;
            *step_complete_f = 1;
        ...
        case SETDIR_BRORAD_REVERSE_DIRECTION:				
            ...
            *step_complete_f = 1;
            ...
            if(ns_dir_bmu_cnt > 0){
						    *afe_phase = SETDIR_INIT_DUMMY_READ;     
							  rcv_check_step = 0;
							  sub_step = ns_dir_bmu_cnt;               
						}
				
            break;		
        case SETDIR_BRORAD_OTP_ECC_TEST_W:
					  fill_data4_payload(d_payload, 0x00,0,0,0);
            drv_bq796xx_command_framing(BROAD_WRITE, 0x00, 0x034C, 1, d_payload, 0);                 
            *before_delay_ms = 2;
            *step_complete_f = 1;			
            sub_step = 0;				
            break;		
        ...
        case SETDIR_AUTO_ADDR:
		        fill_data4_payload(d_payload,dir_sel + 1,0,0,0);						
				    drv_bq796xx_command_framing(BROAD_WRITE, 0x00, 0x0309, 1, d_payload, 0);          
            *before_delay_ms = 1;
            *step_complete_f = 1;
            sub_step = 0; 				
            break;
...
#line 1879 "..\\..\\..\\Drivers\\SMP\\smp_drv_bq796xx.c"
				    fill_data4_payload(d_payload,sub_step,0,0,0);                                 						
        ...
        case SETDIR_SET_TOP:
            if (bmu_cnt > maxcnt ){
                bmu_cnt = maxcnt;
						...
						if(bmu_cnt>= 0x01){
								fill_data4_payload(d_payload,3,0,0,0);
#line 1978 "..\\..\\..\\Drivers\\SMP\\smp_drv_bq796xx.c"
								drv_bq796xx_command_framing(SINGLE_WRITE,  bmu_cnt, 0x0308, 1, d_payload, 0);
