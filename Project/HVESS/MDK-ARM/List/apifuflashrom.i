...
typedef struct{
	tLbyte		UpdateAddress[2];
	DWORD		UpdateAddressTemp;
	DWORD		BaseAddress;
	WORD		FwPackageNum;
	WORD		FwPackageNumCnt;
	tLbyte		FwVersion;
	tLbyte		CodeBufStatus[2];
	BYTE		FwBufIndexSptr;
	BYTE		FwBufIndexEptr;
	BYTE		InfoStatus;
	uint8_t		CodeBuf[2][256] ;
...
static void writeCodeDataToFlashRom(uint32_t addr, uint8_t *pDatBuf, uint16_t leng)
{
	...
	mHalEeProm.StartAddress = (0x08000000L + (256 * 1024L)) + addr;
...
static void checkFuCodeDataSwTimerHandler( void *dest, uint16_t evt, void *vDataPtr)
{
	uint8_t		i;
	uint32_t		buffer[65];
	tHalEeProm		mHalEeProm;
	...
	else if(evt == LIB_SW_TIMER_EVT_SW_10MS_7)
	{
		...
		mHalEeProm.StartAddress += (0x08000000L + (256 * 1024L));
		...
		if(FwUpdateInfo.FwCheck.PackageNumCnt == 0)
		{
			...
			else
			{
				...
				appSerialCanDavinciSendTextMessage("Fw Check error");;
			}
		}
		for(i=0; i<64; i++)
		{
		...
		if(FwUpdateInfo.FwCheck.PackageNumCnt >= FwUpdateInfo.FwPackageNum)
		{
			if(FwUpdateInfo.FwCheck.CheckSum == 0)
			{
				...
				appSerialCanDavinciSendTextMessage("Fw Check success");;
			}
			else
				FwUpdateInfo.FwCheckStatus = API_FU_FW_CHECK_FAIL;
			LibSwTimerClose(checkFuCodeDataSwTimerHandler, 0);
			appSerialCanDavinciSendTextMessage("FW Check Finish");;
		}
		else
		{
...
static void checkRcvFuCodeData(void)
{
	...
	{
		appSerialCanDavinciSendTextMessage("FW 接收完畢,自動檢查是否正確");;
...
void apiFuRcvSetBaseAddr(uint32_t BaseAddr)
{
	...
	
	{
		...
		FwUpdateInfo.InfoStatus |= 0x08;
		

#line 282 "..\\..\\..\\API\\ApiFuFlashRom.c"
	}
}	
...
void apiFuSetUpgradeData(uint32_t address, uint8_t *pDatBuf, uint16_t leng)
{
	...
	{				
			...
			FwUpdateInfo.CodeBufStatus[FwUpdateInfo.FwBufIndexEptr].l == (DWORD)0xffffffffL)	
		{
			...
			FwUpdateInfo.UpdateAddress[FwUpdateInfo.FwBufIndexEptr].b[0] = 0;	
