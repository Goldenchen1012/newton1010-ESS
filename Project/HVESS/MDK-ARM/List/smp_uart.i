...
#line 20 "..\\..\\..\\Drivers\\SMP\\smp_uart.c"
...
#line 21 "..\\..\\..\\Drivers\\SMP\\smp_uart.c"
#line 22 "..\\..\\..\\Drivers\\SMP\\smp_uart.c"
...
#line 23 "..\\..\\..\\Drivers\\SMP\\smp_uart.c"
#line 24 "..\\..\\..\\Drivers\\SMP\\smp_uart.c"
...
uint8_t 									uart0_tx_buffer[256] = {0};
uint8_t 									uart0_rx_buffer[1] = {0};
...
{
	if(p_uart->num == UART0){
		smp_uart0_handle.Instance        = ((USART_TypeDef *) ((0x40000000UL) + 0x4C00UL));
		smp_uart0_handle.Init.BaudRate   = p_uart->baud_rate;

		smp_uart0_handle.Init.WordLength = 0x00000000U;
		smp_uart0_handle.Init.StopBits   = 0x00000000U;
		if(p_uart->use_parity == PARITY_NONE){
			smp_uart0_handle.Init.Parity     = 0x00000000U;
		}else if(p_uart->use_parity == PARITY_EVEN){
			smp_uart0_handle.Init.Parity     = (0x1UL << (10U));
		}else if(p_uart->use_parity == PARITY_ODD){
			smp_uart0_handle.Init.Parity     = ((0x1UL << (10U)) | (0x1UL << (9U)));
		}else{
			return ((0x0) - 3);
		}
		if(p_uart->flow_ctrl == UART_FLOW_CTRL_DISABLE){
			smp_uart0_handle.Init.HwFlowCtl  = 0x00000000U;
		}else if(p_uart->flow_ctrl == UART_FLOW_CTRL_ENABLE){
			smp_uart0_handle.Init.HwFlowCtl  = ((0x1UL << (8U)) | (0x1UL << (9U)));
		}else{
			return ((0x0) - 3);
		}
			
		smp_uart0_handle.Init.Mode       = ((0x1UL << (3U)) |(0x1UL << (2U)));
		...
		if(HAL_UART_Receive_DMA(&smp_uart0_handle, uart0_rx_buffer, 1) != HAL_OK){
				return ((0x0) - 1);
		}
	}else	if(p_uart->num == UART1){
		smp_uart1_handle.Instance        = ((USART_TypeDef *) ((0x40000000UL) + 0x4800UL));
		smp_uart1_handle.Init.BaudRate   = p_uart->baud_rate;

		smp_uart1_handle.Init.WordLength = 0x00000000U;
		smp_uart1_handle.Init.StopBits   = 0x00000000U;
		
		if(p_uart->use_parity == PARITY_NONE){
			smp_uart1_handle.Init.Parity     = 0x00000000U;
		}else if(p_uart->use_parity == PARITY_EVEN){
			smp_uart1_handle.Init.Parity     = (0x1UL << (10U));
		}else if(p_uart->use_parity == PARITY_ODD){
			smp_uart1_handle.Init.Parity     = ((0x1UL << (10U)) | (0x1UL << (9U)));
		}else{
			return ((0x0) - 3);
		}
		if(p_uart->flow_ctrl == UART_FLOW_CTRL_DISABLE){
			smp_uart1_handle.Init.HwFlowCtl  = 0x00000000U;
		}else if(p_uart->flow_ctrl == UART_FLOW_CTRL_ENABLE){
			smp_uart1_handle.Init.HwFlowCtl  = ((0x1UL << (8U)) | (0x1UL << (9U)));
		}else{
			return ((0x0) - 3);
		}
			
		smp_uart1_handle.Init.Mode       = ((0x1UL << (3U)) |(0x1UL << (2U)));
		...
		HAL_UART_Receive_IT(&smp_uart1_handle, uart1_rx_buffer, 1);
...
int8_t smp_uart_put(smp_uart_t *p_uart, uint8_t byte)
{
	uint16_t size = 0, i =0;
	if(p_uart->num == UART0){
        if (smp_fifo_push(&uart0_tx_fifo, byte) == ((0x0) - 0)){
        
        
        
        
            if((smp_uart0_handle.gState == 0x00000020U)||(smp_uart0_handle.gState == 0x00000022U)){
				        if(smp_fifo_get_size(&uart0_tx_fifo,&size)==((0x0) - 0)){
        ...
        if (smp_fifo_push(&uart1_tx_fifo, byte) == ((0x0) - 0)){
        
        
        
        
			      if((smp_uart1_handle.gState == 0x00000020U)||(smp_uart1_handle.gState == 0x00000022U)){
				        if(smp_fifo_get_size(&uart1_tx_fifo,&size)==((0x0) - 0)){
					          if(size>256) size = 256;
...
{
	uint16_t size = 0, i =0;
	if(huart->Instance == ((USART_TypeDef *) ((0x40000000UL) + 0x4C00UL))){
		if(smp_fifo_get_size(&uart0_tx_fifo,&size)==((0x0) - 0)){
			if(size>256)
				size = 256;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	if(huart->Instance == ((USART_TypeDef *) ((0x40000000UL) + 0x4C00UL))){
		...
		if(smp_fifo_push(&uart0_rx_fifo, (char)uart0_rx_buffer[0]) == ((0x0) - 0)){
			flag = 1;
		}else{
			flag = 0;
		}		
			
		if(HAL_UART_Receive_DMA(huart, uart0_rx_buffer, 1) != HAL_OK){
			return;
		}
		...
		if(smp_fifo_push(&uart1_rx_fifo, (char)uart1_rx_buffer[0]) == ((0x0) - 0)){
			flag = 1;
		}else{
			flag = 0;
		}		
			
		if(HAL_UART_Receive_IT(huart, uart1_rx_buffer, 1) != HAL_OK){
			return;
		}
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
	...
	if(huart->Instance==((USART_TypeDef *) ((0x40000000UL) + 0x4C00UL))){
		...
		do { volatile uint32_t tmpreg; ((((RCC_TypeDef *) (((0x40000000UL) + 0x00020000UL) + 0x1000UL))->APB1ENR1) |= ((0x1UL << (19U)))); tmpreg = ((((RCC_TypeDef *) (((0x40000000UL) + 0x00020000UL) + 0x1000UL))->APB1ENR1) & ((0x1UL << (19U)))); (void)tmpreg; } while(0);
		...
		GPIO_InitStruct.Alternate = ((uint8_t)0x08) | ((uint8_t)0x08);
		...
		uart0_dma_tx.Instance                 = ((DMA_Channel_TypeDef *) ((((0x40000000UL) + 0x00020000UL) + 0x0400UL) + 0x0030UL));
		uart0_dma_tx.Init.Direction           = (0x1UL << (4U));
		uart0_dma_tx.Init.PeriphInc           = 0x00000000U;
		uart0_dma_tx.Init.MemInc              = (0x1UL << (7U));
		uart0_dma_tx.Init.PeriphDataAlignment = 0x00000000U;
		uart0_dma_tx.Init.MemDataAlignment    = 0x00000000U;
		uart0_dma_tx.Init.Mode                = 0x00000000U;
		uart0_dma_tx.Init.Priority            = 0x00000000U;
		uart0_dma_tx.Init.Request             = 2U;

		HAL_DMA_Init(&uart0_dma_tx);

		 
		do{ (huart)->hdmatx = &(uart0_dma_tx); (uart0_dma_tx). Parent = (huart); } while(0);

		 
		uart0_dma_rx.Instance                 = ((DMA_Channel_TypeDef *) ((((0x40000000UL) + 0x00020000UL) + 0x0400UL) + 0x0058UL));
		uart0_dma_rx.Init.Direction           = 0x00000000U;
		uart0_dma_rx.Init.PeriphInc           = 0x00000000U;
		uart0_dma_rx.Init.MemInc              = (0x1UL << (7U));
		uart0_dma_rx.Init.PeriphDataAlignment = 0x00000000U;
		uart0_dma_rx.Init.MemDataAlignment    = 0x00000000U;
		uart0_dma_rx.Init.Mode                = 0x00000000U;
		uart0_dma_rx.Init.Priority            = (0x2UL << (12U));
		uart0_dma_rx.Init.Request             = 2U;

		HAL_DMA_Init(&uart0_dma_rx);

		 
		do{ (huart)->hdmarx = &(uart0_dma_rx); (uart0_dma_rx). Parent = (huart); } while(0);
		...
		GPIO_InitStruct.Pull      = (0x00000001u);
		GPIO_InitStruct.Speed     = (0x00000003u);
		GPIO_InitStruct.Alternate = ((uint8_t)0x07) | ((uint8_t)0x07);

		HAL_GPIO_Init(((GPIO_TypeDef *) (((0x40000000UL) + 0x08000000UL) + 0x0400UL)), &GPIO_InitStruct);

		 
		 
#line 480 "..\\..\\..\\Drivers\\SMP\\smp_uart.c"
		
		 
#line 507 "..\\..\\..\\Drivers\\SMP\\smp_uart.c"
...
void HAL_UART_MspDeInit(UART_HandleTypeDef *huart)
{
	if(huart->Instance == ((USART_TypeDef *) ((0x40000000UL) + 0x4C00UL))){
		  ...
		  ((((RCC_TypeDef *) (((0x40000000UL) + 0x00020000UL) + 0x1000UL))->APB1RSTR1) |= ((0x1UL << (19U))));
		  ((((RCC_TypeDef *) (((0x40000000UL) + 0x00020000UL) + 0x1000UL))->APB1RSTR1) &= ~((0x1UL << (19U))));

		   
		   
		  HAL_GPIO_DeInit(((GPIO_TypeDef *) (((0x40000000UL) + 0x08000000UL) + 0x0800UL)), ((uint16_t)0x0400));
		   
		  HAL_GPIO_DeInit(((GPIO_TypeDef *) (((0x40000000UL) + 0x08000000UL) + 0x0800UL)), ((uint16_t)0x0800));
		  ...
		  ((((RCC_TypeDef *) (((0x40000000UL) + 0x00020000UL) + 0x1000UL))->APB1RSTR1) |= ((0x1UL << (18U))));
		  ((((RCC_TypeDef *) (((0x40000000UL) + 0x00020000UL) + 0x1000UL))->APB1RSTR1) &= ~((0x1UL << (18U))));

		   
		   
		  HAL_GPIO_DeInit(((GPIO_TypeDef *) (((0x40000000UL) + 0x08000000UL) + 0x0400UL)), ((uint16_t)0x0400));
		   
		  HAL_GPIO_DeInit(((GPIO_TypeDef *) (((0x40000000UL) + 0x08000000UL) + 0x0400UL)), ((uint16_t)0x0800));
