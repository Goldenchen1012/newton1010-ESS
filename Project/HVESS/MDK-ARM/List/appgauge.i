...
#line 29 "..\\..\\..\\APP\\AppGauge.c"
#line 30 "..\\..\\..\\APP\\AppGauge.c"
...
typedef struct{
	...
	tLLbyte			RM;				
	tLbyte			MaxInputPower;		
	tLbyte			MaxOutputPower;
	tLbyte			CalMaxInputPower;	
	tLbyte			CalMaxOutputPower;
	WORD			Table_RaValue;	
	WORD			Cal_RaValue;	
	BYTE			TerminateVoltageCount;	
	BYTE			FullChargeTimerCount;	
	BYTE			CalRaIdleTimer;			
	BYTE			Over5HrStatus;
	tLLbyte			AccChgPower;		
	tLLbyte			AccDhgPower;		
...
{
	mAppScuGauage.ChargeMode = 0;
	mAppScuGauage.ChargeModeTemp = 3;
}
...
DWORD CvtQmaxPassChargeTomAh(void)
{
	...
	LLbyte.sll /= (DLONG)(3600u * 1000u * 1000u);    
	return	LLbyte.l[0];
}
...
DWORD appScuGaugeCvtRmTomAh(void)
{
	...
	LLbyte.ll /= (DDWORD)(3600u * 1000u * 1000u);    
	return	LLbyte.l[0];
}
...
uint16_t GetAverageCellVoltage(void)
{
	...
	TotalCellVoltage /= apiSysParGetCellNumber();
...
WORD GetRaFromSoc(WORD SOC)
{
	...
	Ra = mAppScuGauage.RaTable[0].Value;	
...
static void gaugeCalCellRValue(void)
{
	...
	else if(appGaugeGetCurrentMode() == 2)
	{
...
void appScuGaugeUpdateQmax(DWORD passcharge,BYTE Times)
{
	...
	mAppScuGauage.QmaxuAms.ll *= 1000;	
	mAppScuGauage.QmaxuAms.ll *= 3600;	
	mAppScuGauage.QmaxuAms.ll *= 1000;	
	
	;
	mAppScuGauage.QmaxStatus = 0;	
	gaugeQPassChargeIni();			
...
static uint8_t	gaugeUpdateCurrentMode(void)
{
	...
	{
		if(Current < 0)
			mAppScuGauage.ChargeMode = 2;
		else
			mAppScuGauage.ChargeMode = 1;
	}
...
static void gaugeCalCapacity(void)
{
	...
	else if(appGaugeGetCurrentMode() == 1)
	{
		...
		mAppScuGauage.TotalDisChargeCount.ll += CalRmValue;	
		mAppScuGauage.RaPassCharge.sll -= CalRmValue;
	}
}
...
uint16_t getSocUseVoltage(uint32_t voltage)
{
	...
	if(voltage < mAppScuGauage.OcvTable[0].Value)
		...
		soc = (uint16_t)mAppScuGauage.OcvTable[mAppScuGauage.OcvTableNum - 1].Level * 100;		
	else
	{
		for(i=0; i<(mAppScuGauage.OcvTableNum - 1); i++)
		{
...
void getSocTableUseAvgCellVoltage(void)
{
	   ...
	   voltage < apiSysParGetMaxFlatVoltage())
	{
		...
		appSerialCanDavinciSendTextMessage("¥­©Z°Ï");
		return;
	}
...
static void gaugeReleaseMode(void)
{
	...
	if(mAppScuGauage.ReadSocTime >= (30*60) )		
	{
		
		mAppScuGauage.ReadSocTime = (30*60-100);	
		...
		if(mAppScuGauage.RelaxSec >= (5L*3600L))	
		{
		...
		if(mAppScuGauage.NowSocValid)	
		{
			...
			mAppScuGauage.StartCalRaSoc = mAppScuGauage.Soc0;	
		...
		if(mAppScuGauage.QmaxStatus == 1)	
		{
			...
			if(mAppScuGauage.QmaxUpdateTimes.i == 0)	
			{
...
WORD CheckTerminateSocUseRa(void)
{
	...
	basesoc = getTerminateSocFromOcvTable();	
	lastsoc = basesoc;
	endsoc = 10000;
	SocStep = 1000;
...
void gaugeCheckCycleCount(void)
{
	...
	LLbyte.ll *= (DDWORD)(3600000u * 800u);		
...
static void scuGaugeUnReleasedMode(void)
{
	...
	{	
		...
		
		{
			...
			if(v < mAppScuGauage.TerminateVoltage)	
			{
				mAppScuGauage.TerminateVoltageCount++;
				if(mAppScuGauage.TerminateVoltageCount >= 40)	
				{
	...
	else		
	{
		...
		
		{
			...
			if(v >= mAppScuGauage.FullCharge.Voltage && 
			   current < mAppScuGauage.FullCharge.Current)	
			{
...
#line 1224 "..\\..\\..\\APP\\AppGauge.c"
					...
					d = mAppScuGauage.Qmax.l;	
					d *= 1000;	
					mAppScuGauage.RM.ll = (DDWORD)d*(DDWORD)3600*(DDWORD)1000;	
		...
		mAppScuGauage.RaLastSoc = 0xffff;		
...
{
	mAppScuGauage.TotalDisChargeCount.ll = apiRamLoadTotalDisChargeCount();	
	mAppScuGauage.TotalDisChargeCount.ll *= (3600u * 1000u *1000u);	
}
...
static void gaugeSwTimerHandler( void *dest, uint16_t evt, void *vDataPtr)
{
...
static void gaugeHwTimerHandler( void *dest, uint16_t evt, void *vDataPtr)
{
...
void appGaugeSetRM(uint32_t rm)
{
	...
	mAppScuGauage.RM.ll *= (DDWORD)(3600u * 1000u * 1000u);    
}
...
void appGaugeOpen(tLibRegisterEvtHandler eventHandler)
{
	chargeModeIni();
	gaugeQPassChargeIni();
	gaugeRPassChargeIni();
