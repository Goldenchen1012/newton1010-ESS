...
#line 30 "..\\..\\..\\API\\ApiSysPar.c"
#line 31 "..\\..\\..\\API\\ApiSysPar.c"
...
static void saveBatteryInfoPar(void)
{
	...
	memcpy(&mBatteryInfo.HeadInfo, "BatPar01", 8);
	mBatteryInfo.ParLeng = sizeof(tBatteryInfo);
	mBatteryInfo.DateCode = 0x1108;
	...
	mHalEeProm.StartAddress = (0x08000000L + 506L * 1024L);
	mHalEeProm.Length = mBatteryInfo.ParLeng;
	mHalEeProm.pDataBuffer = (uint8_t *)&mBatteryInfo;
	
	if(HalEePromErase(&mHalEeProm) != 0)
		appSerialCanDavinciSendTextMessage((uint8_t *)"Save batinfo Par ..erase error");
	
	HalEePromWrite(&mHalEeProm);
	
	appSerialCanDavinciSendTextMessage((uint8_t *)"Save batinfo Par");
}
...
static void loadBatteryInfoPar(void)
{
	...
	appSerialCanDavinciSendTextMessage((uint8_t *)"Load bat Par");
	HalEePromRead(&mHalEeProm);
	
	
	
	pBatteryInfo = (tBatteryInfo *)buffer;
	if(memcmp(pBatteryInfo->HeadInfo, "BatPar01", 8) != 0)
	{
		appSerialCanDavinciSendTextMessage((uint8_t *)"bat info Head Info Error");	
		flag = 1;
	}
	else if(pBatteryInfo->ParLeng != sizeof(tBatteryInfo))
	{
		appSerialCanDavinciSendTextMessage((uint8_t *)"batinfo len Error");	
		flag = 1;
	}
	else if(pBatteryInfo->DateCode != 0x1108)
	{
		appSerialCanDavinciSendTextMessage((uint8_t *)"bat info DateCode Error");	
		flag = 1;
	}
	if(!flag)
	{
		...
		{	
			...
			appSerialCanDavinciSendTextMessage((uint8_t *)str);
		}
	}
	if(flag)
	{
	...
	{
		appSerialCanDavinciSendTextMessage((uint8_t *)"BatInfo Load Par ok");		
...
static void batteryInfoSwTimerHandler( void *dest, uint16_t evt, void *vDataPtr)
{
...
static uint16_t loadSysPar(void)
{
	...
	appSerialCanDavinciSendTextMessage((uint8_t *)"Load Par");
	HalEePromRead(&mHalEeProm);
	
	
	
	pSysPar = (tSysPar *)buffer;
	if(memcmp(pSysPar->RomPar.HeadInfo, "SysPar01", 8) != 0)
	{
		appSerialCanDavinciSendTextMessage((uint8_t *)"Head Info Error");	
		flag = 1;
	}
	else if(pSysPar->RomPar.ParLeng != sizeof(tSysRomPar))
	{
		appSerialCanDavinciSendTextMessage((uint8_t *)"len Error");	
		flag = 1;
	}
	else if(pSysPar->RomPar.DateCode != 0x1214)
	{
		appSerialCanDavinciSendTextMessage((uint8_t *)"DateCode Error");	
		flag = 1;
	}
	if(!flag)
	{
	...
	{
		appSerialCanDavinciSendTextMessage((uint8_t *)"Load Par ok");		
...
static void saveSysPar(void)
{
	...
	memcpy(&SystemParemater.RomPar.HeadInfo, "SysPar01", 8);
	SystemParemater.RomPar.ParLeng = sizeof(tSysRomPar);
	SystemParemater.RomPar.DateCode = 0x1214;
	...
	if(HalEePromErase(&mHalEeProm) != 0)
			appSerialCanDavinciSendTextMessage((uint8_t *)"Save Sys Par ..erase error");
	
	HalEePromWrite(&mHalEeProm);
	
	appSerialCanDavinciSendTextMessage((uint8_t *)"Save Sys Par");
}
...
static void loadCalPar(void)
{
	...
	mHalEeProm.StartAddress = (0x08000000L + 508L * 1024L);
	mHalEeProm.Length = sizeof(tSysCalPar);
	mHalEeProm.pDataBuffer = buffer;
	
	appSerialCanDavinciSendTextMessage((uint8_t *)"Load cal Par");
	HalEePromRead(&mHalEeProm);
	
	
	
	pSysCalPar = (tSysCalPar *)buffer;
	if(memcmp(pSysCalPar->RomPar.HeadInfo, "CalPar01", 8) != 0)
	{
		appSerialCanDavinciSendTextMessage((uint8_t *)"Cal Head Info Error");	
		flag = 1;
	}
	else if(pSysCalPar->RomPar.ParLeng != sizeof(tCalRomPar))
	{
		appSerialCanDavinciSendTextMessage((uint8_t *)"Cal len Error");	
		flag = 1;
	}
	else if(pSysCalPar->RomPar.DateCode != 0x1102)
	{
		appSerialCanDavinciSendTextMessage((uint8_t *)"Cal DateCode Error");	
		flag = 1;
	}
	if(!flag)
	{
...
static void saveCalPar(void)
{
	...
	memcpy(&SysCalPar.RomPar.HeadInfo, "CalPar01", 8);
	SysCalPar.RomPar.ParLeng = sizeof(tCalRomPar);
	SysCalPar.RomPar.DateCode = 0x1102;
	...
	if(HalEePromErase(&mHalEeProm) != 0)
			appSerialCanDavinciSendTextMessage((uint8_t *)"Save Cal Par ..erase error");
	
	HalEePromWrite(&mHalEeProm);
	
	appSerialCanDavinciSendTextMessage((uint8_t *)"Save Cal Par");
}
...
static void loadScuIdPar(void)
{
	...
	mHalEeProm.StartAddress = (0x08000000L + 504L * 1024L);
	mHalEeProm.Length = sizeof(tScuIdInfo);
	mHalEeProm.pDataBuffer = buffer;
	
	appSerialCanDavinciSendTextMessage((uint8_t *)"Load scuid Par");
	HalEePromRead(&mHalEeProm);
	
	
	
	pScuIdInfo = (tScuIdInfo *)buffer;
	if(memcmp(pScuIdInfo->HeadInfo, "ScuID001", 8) != 0)
	{
		appSerialCanDavinciSendTextMessage((uint8_t *)"scuid Head Info Error");	
		flag = 1;
	}
	else if(pScuIdInfo->ParLeng != sizeof(tScuIdInfo))
	{
		appSerialCanDavinciSendTextMessage((uint8_t *)"scuid len Error");	
		flag = 1;
	}
	else if(pScuIdInfo->DateCode != 0x2107)
	{
		appSerialCanDavinciSendTextMessage((uint8_t *)"scuid DateCode Error");	
		flag = 1;
	}
	if(!flag)
	{
	...
	{
		appSerialCanDavinciSendTextMessage((uint8_t *)"Load scuid ok");		
...
void saveScuIdPar(uint8_t scuid)
{
	...
	memcpy(&mScuIdInfo.HeadInfo, "ScuID001", 8);
	mScuIdInfo.ParLeng = sizeof(tScuIdInfo);
	mScuIdInfo.DateCode = 0x2107;
	...
	if(HalEePromErase(&mHalEeProm) != 0)
		appSerialCanDavinciSendTextMessage((uint8_t *)"Save scuid Par ..erase error");
	
	HalEePromWrite(&mHalEeProm);
	
	appSerialCanDavinciSendTextMessage((uint8_t *)"Save scuid Par");
}


static void sysParSwTimerHandler( void *dest, uint16_t evt, void *vDataPtr)
{
...
void apiSysParSetQmaxUpdateTimes(uint16_t times)
{
	appSerialCanDavinciSendTextMessage((uint8_t *)"apiSysParSetQmaxUpdateTimes");
...
void apiSysParSetCycleCount(uint16_t count)
{
	appSerialCanDavinciSendTextMessage((uint8_t *)"apiSysParSetCycleCount");
...
void apiSysParSetPfFlag(uint16_t flag)
{
	appSerialCanDavinciSendTextMessage((uint8_t *)"apiSysParSetPfFlag");
